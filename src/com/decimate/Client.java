...
import com.decimate.content.screenshot.Screenshot;
import com.decimate.content.youtube.Youtube;
import com.decimate.content.youtube.YoutubeChannel;
import com.decimate.content.youtube.YoutubeVideo;
import com.decimate.dtos.ItemDto;
import com.decimate.features.BroadCast;
import com.decimate.features.OverlayTimersMgr;
import com.decimate.interfaces.MysteryBoxInterface;
import com.decimate.itemstats.ItemDescription;
import com.decimate.itemstats.ItemStats;
...
public class Client extends RSApplet {
    ...
    public static boolean CACHED_FOG_SETTING = true;

    public static final Random RANDOM = new Random();

    public static int getAnimatedIconId(int rights) {
        switch (rights) {
            case 5: // sapphire
                return 5;
            case 6: // emerald
     ...
     *
     */
    public static Sprite[] animatedRankIcon;
    private final static int SHADOW_SPEED = 1;
    private static final int[] levelXPs;
    private static final String VALID_MONEY_POUCH_KEYS = "KMBQkmbq0123456789";
    ...
    public static int clientWidth = 765, clientWidth2 = 765, clientHeight = 503, clientHeight2 = 503;
    ...
    private final byte[] chatRights;
    private final byte[] chatIronman;
    ...
    public final int[] myAppearanceColors;
    ...
    private OverlayTimersMgr overlayTimersMgr;
    /*
     * Money orb
     */
    ...
    private boolean loginButtonHover, discordButtonHover, youtubeButtonHover, input1Hover, input2Hover;
    ...
    public Client() {
        ...
        autoBackgroundSprites = new Sprite[9];
        ...
        bigX = new int[4000];
        bigY = new int[4000];
        overlayTimersMgr = new OverlayTimersMgr();
    }
    ...
    public static void drawBorder(Sprite backTexture, int x, int y, int width, int height, int alpha, int backColour,
        ...
        final int pieceWidth = spriteLoader.lookup(623).myWidth;
        final int pieceHeight = spriteLoader.lookup(625).myHeight;
        ...
        spriteLoader.lookup(622).drawAdvancedSprite(x, y);
        spriteLoader.lookup(624).drawAdvancedSprite(x + width - remainingWidth, y);
        spriteLoader.lookup(626).drawAdvancedSprite(x + width - remainingWidth, y + height - remainingHeight);
        spriteLoader.lookup(628).drawAdvancedSprite(x, y + height - remainingHeight);

        for (int horizontalIndex = 1; horizontalIndex < (width / pieceWidth); horizontalIndex++) {
            spriteLoader.lookup(623).drawAdvancedSprite(x + (horizontalIndex * pieceWidth), y);// top
            spriteLoader.lookup(627).drawAdvancedSprite(x + (horizontalIndex * pieceWidth),
                    y + height - remainingHeight);// bottom
        }

        for (int verticalIndex = 2; verticalIndex < (height / pieceHeight); verticalIndex++) {
            spriteLoader.lookup(625).drawAdvancedSprite(x + width - remainingWidth, y + (verticalIndex * pieceHeight));// right
            spriteLoader.lookup(629).drawAdvancedSprite(x, y + (verticalIndex * pieceHeight));// left
        }
    }
    ...
    public static void resolveProgramArgs(String[] args) {
        if (args.length == 0) {
            return;
        }
    ...
    private static void processProgramArg(String key, List<String> values) {
        switch (key) {
            case "dev": {
                assert values.size() == 0;
                Configuration.SERVER_HOST = "127.0.0.1";
                Configuration.localHost = true;
                break;
            }
            case "test": {
                assert values.size() == 0;
                Configuration.localHost = false;
                Configuration.PORT += 1;
                CacheDownloader.CACHE_NAME = "DecimateCache-testing";
                break;
            }
        }
    }
    ...
    private void handleSettings() {
        if (getOption("hd_tex")) {
            int l2 = RSInterface.interfaceCache.get(35616).valueIndexArray[0][1];
            variousSettings[l2] = 1 - variousSettings[l2];
        }
        if (getOption("filter_spam")) {
            int l2 = RSInterface.interfaceCache.get(35613).valueIndexArray[0][1];
            variousSettings[l2] = 1 - variousSettings[l2];
        }
        if (getOption("old_hits")) {
            int l2 = RSInterface.interfaceCache.get(35568).valueIndexArray[0][1];
            variousSettings[l2] = 1 - variousSettings[l2];
        }
        if (getOption("constitution")) {
            int l2 = RSInterface.interfaceCache.get(35571).valueIndexArray[0][1];
            variousSettings[l2] = 1 - variousSettings[l2];
        }
        if (getOption("cursors")) {
            int l2 = RSInterface.interfaceCache.get(35584).valueIndexArray[0][1];
            variousSettings[l2] = 1 - variousSettings[l2];
        }
        if (getOption("old_frame")) {
            int l2 = RSInterface.interfaceCache.get(35565).valueIndexArray[0][1];
            variousSettings[l2] = 1 - variousSettings[l2];
        }
        if (getOption("rare_drops")) {
            int l2 = RSInterface.interfaceCache.get(35587).valueIndexArray[0][1];
            variousSettings[l2] = 1 - variousSettings[l2];
        }
        if (getOption("censor_active")) {
            int l2 = RSInterface.interfaceCache.get(35590).valueIndexArray[0][1];
            variousSettings[l2] = 1 - variousSettings[l2];
        }
        if (getOption("fog_active")) {
            int l2 = RSInterface.interfaceCache.get(35619).valueIndexArray[0][1];
            variousSettings[l2] = 1 - variousSettings[l2];
        }
        if (getOption("absorb_damage")) {
            int l2 = RSInterface.interfaceCache.get(35643).valueIndexArray[0][1];
            variousSettings[l2] = 1 - variousSettings[l2];
        }
        if (getOption("save_input")) {
            int l2 = RSInterface.interfaceCache.get(35646).valueIndexArray[0][1];
            variousSettings[l2] = 1 - variousSettings[l2];
        }
        if (getOption("ground_blending")) {
            int l2 = RSInterface.interfaceCache.get(35649).valueIndexArray[0][1];
            variousSettings[l2] = 1 - variousSettings[l2];
        }
        if (!musicEnabled)
            stopMidi();

        CACHED_FOG_SETTING = getOption("fog_active");
    }
...
//            musicEnabled = in.readByte() == 1;
            ...
            options.put("rare_drops", in.readByte() == 1);
            ...
            options.put("ground_blending", in.readByte() == 1);
            chatLoot = in.readByte() == 1;
    ...
    public void saveSettings() {
        try {
            ...
            out.writeUTF(myUsername == null ? "" : myUsername);
            out.writeUTF(myPassword == null ? "" : myPassword);
...
//            out.write(musicEnabled ? 1 : 0);
            ...
            out.write(getOption("rare_drops") ? 1 : 0);
            ...
            out.write(getOption("ground_blending") ? 1 : 0);
            out.write(chatLoot ? 1 : 0);
    ...
    public void updateGameArea() {
        try {
            ...
            Rasterizer.fieldOfView = gameAreaWidth * gameAreaHeight / 85504 << 1;

            boolean fixed = clientSize == 0;
            if (clientZoom > (fixed ? 1100 : 2200)) {
                clientZoom = (fixed ? 1100 : 2200);
            } else if (clientZoom < (fixed ? 180 : 240)) {
                clientZoom = (fixed ? 180 : 240);
            }

            WorldController.setupViewport(500, 800, gameAreaWidth,
                    gameAreaHeight, ai);
            if (loggedIn) {
                initGamePoducers(true);
            } else {
                titleScreen = new RSImageProducer(clientWidth, clientHeight, 0, 0);
            }
        } catch (Exception e) {
            e.printStackTrace();
        }
    ...
    public int getPrefixSubstringLength(String prefix) {
        ...
        if ((prefix.contains("cr1") && !prefix.contains("@cr1@")) && amount == 2) {
            length = 6;
        } else if (prefix.contains("cr")) {
            length = 5;
        }
        return length;
    }
    ...
    public void drawChatArea() {
        ...
        if (showChat) {
            spriteLoader.lookup(0).drawSprite(0 + offsetX, 0 + offsetY);
        }
        drawChannelButtons(offsetX, offsetY);
        if (showInput) {
            spriteLoader.lookup(64).drawSprite(0 + offsetX, 0 + offsetY);
            ...
            spriteLoader.lookup(64).drawSprite(0 + offsetX, 0 + offsetY);
            newBoldFont.drawCenteredString(inputTitle != null ? inputTitle : "Enter amount:", 259 + offsetX, 60 + offsetY, 0, -1);
            newBoldFont.drawCenteredString(amountOrNameInput + "*", 259 + offsetX, 80 + offsetY, 128, -1);
        } else if (inputDialogState == 2) {
            spriteLoader.lookup(64).drawSprite(0 + offsetX, 0 + offsetY);
            newBoldFont.drawCenteredString(inputTitle != null ? inputTitle : "Enter name:", 259 + offsetX, 60 + offsetY, 0, -1);
            newBoldFont.drawCenteredString(amountOrNameInput + "*", 259 + offsetX, 80 + offsetY, 128, -1);
            ...
            spriteLoader.lookup(64).drawSprite(0 + offsetX, 0 + offsetY);
            drawInterface(0, 20 + offsetX, RSInterface.interfaceCache.get(backDialogID), 20 + offsetY);
        } else if (dialogID != -1) {
            spriteLoader.lookup(64).drawSprite(0 + offsetX, 0 + offsetY);
            drawInterface(0, 20 + offsetX, RSInterface.interfaceCache.get(dialogID), 20 + offsetY);
            ...
            for (int index = 0; index < 500; index++) {
                ...
                if (chatMessages[index] != null) {
                    ...
                    byte playerRights = chatRights[index];
                    byte ironman2 = chatIronman[index];

                    if (name != null && name.startsWith("@glo")) {
                        name = name.substring(7);
                    }
                    if (chatType == 0 || chatType == 23) {
                        if (chatTypeView == 5 || chatTypeView == 0) {
                            if(chatType == 23 && options.get("filter_spam"))
                                continue;
                            if (positionY > 0 && positionY < 210) {
                                int xPos = 11;
                                newRegularFont.drawBasicString(chatMessages[index], xPos + offsetX, positionY + offsetY, 0, -1);
                            }
                            scrollPosition++;
                            messageY++;
                        }
                    ...
                    if ((chatType == 1 || chatType == 2)
                        ...
                        if (chatTypeView == 1 || chatTypeView == 0 || (playerRights > 0 && playerRights <= 4)) {
                            if (positionY > 0 && positionY < 210) {
                                ...
                                if (ironman2 > 0) {
                                    spriteLoader.lookup(710 + ironman2).drawSprite(xPos + 1 + offsetX, positionY - 11 + offsetY);
                                    xPos += 19;
                                }
                                ...
                                newRegularFont.drawBasicString(chatMessages[index], xPos + offsetX, positionY + offsetY, 255, -1);
                            }
                            scrollPosition++;
                            messageY++;
                        }
                    ...
                    if ((chatType == 3 || chatType == 7) && (splitPrivateChat == 0 || chatTypeView == 2)
                        ...
                        if (chatTypeView == 2 || chatTypeView == 0) {
                            if (positionY > 0 && positionY < 210) {
                                ...
                                newRegularFont.drawBasicString(name + ":", xPos + offsetX, positionY + offsetY, SPLIT_CHAT_COLORS[splitChatColor], -1);
                                xPos += newRegularFont.getTextWidth(name) + 8;
                                newRegularFont.drawBasicString(chatMessages[index], xPos + offsetX, positionY + offsetY, SPLIT_CHAT_COLORS[splitChatColor], -1);
                            }
                            scrollPosition++;
                            messageY++;
                        }
                    ...
                    if (chatType == 4 && (tradeMode == 0 || tradeMode == 1 && isFriendOrSelf(name))) {
                        if (chatTypeView == 3 || chatTypeView == 0) {
                            if (positionY > 0 && positionY < 210) {
                                newRegularFont.drawBasicString(name + " " + chatMessages[index], 11 + offsetX, positionY + offsetY, 0x800080, -1);
                            }
                            scrollPosition++;
                            messageY++;
                        }
                    ...
                    if (chatType == 5 && splitPrivateChat == 0 && privateChatMode < 2) {
                        if (chatTypeView == 2 || chatTypeView == 0) {
                            if (positionY > 0 && positionY < 210) {
                                newRegularFont.drawBasicString(chatMessages[index], 11 + offsetX, positionY + offsetY, 0x800000, -1);
                            }
                            scrollPosition++;
                            messageY++;
                        }
                    ...
                    if (chatType == 6 && (splitPrivateChat == 0 || chatTypeView == 2) && privateChatMode < 2) {
                        if (chatTypeView == 2 || chatTypeView == 0) {
                            if (positionY > 0 && positionY < 210) {
                                newRegularFont.drawBasicString("To " + name + ":", 11 + offsetX, positionY + offsetY, SPLIT_CHAT_COLORS[splitChatColor], -1);
                                newRegularFont.drawBasicString(chatMessages[index], 15 + newRegularFont.getTextWidth("To :" + name) + offsetX + offsetX, positionY + offsetY, SPLIT_CHAT_COLORS[splitChatColor], -1);
                            }
                            scrollPosition++;
                            messageY++;
                        }
                    ...
                    // Drop loot announcements
                    else if (chatType == 18) {
                        if (chatTypeView == 3 || chatTypeView == 0 && chatLoot) {
                            if (positionY > 0 && positionY < 210) {
                                newRegularFont.drawBasicString(name + " " + chatMessages[index], 11 + offsetX, positionY + offsetY, 0, -1);
                            }
                            scrollPosition++;
                            messageY++;
                        }
            ...
            if (ironman > 0) {
                spriteLoader.lookup(710 + ironman).drawSprite(12 + offsetX, 122 + offsetY);
                offsetX += 20;
            }
            ...
            spriteLoader.lookup(14).drawSprite(textDrawingArea.getTextWidth(name) + 11 + offsetX, 123 + offsetY);
            textDrawingArea.drawRegularText(false, textDrawingArea.getTextWidth(name) + 24 + offsetX, 0, ": ", 133 + offsetY);
            newRegularFont.drawBasicString(inputString + "*", 24 + newRegularFont.getTextWidth(name + ": ") + offsetX, 133 + offsetY, 255, -1);
            drawSplitChatSelectionBox();
            drawClanChatSelectionBox();
        ...
        if (clientSize == 0) {
            chatAreaIP.drawGraphics(super.graphics);
        }
    ...
    private boolean clickSplitChatSelectionBox() {
        int splitBoxX = 480;
        int splitBoxY = 122 + (clientSize == 0 ? 339 : clientHeight - 165);
        if (clickMode2 == 1 && mouseX >= splitBoxX && mouseX <= splitBoxX + 16
                && mouseY >= splitBoxY && mouseY <= splitBoxY + 13) {
            clickMode2 = 0;
            splitChatColor = splitChatColor + 1 == SPLIT_CHAT_COLORS.length ? 0
                    : splitChatColor + 1;
            //Save.settings(Client.getClient());
            pushMessage("You've changed your private split-chat color.", 0, "");
            return true;
        }
        return false;
    }

    private void drawSplitChatSelectionBox() {
        try {
            int splitBoxX = 480;
            int heightY = clientSize == 0 ? 0 : clientHeight - 165;
            int splitBoxY = 122 + heightY;
            DrawingArea.fillRect(splitBoxX + 1, splitBoxY + 1, 15, 12,
                    SPLIT_CHAT_COLORS[splitChatColor], 255);
            DrawingArea.fillPixels(splitBoxX, 16, 13, 0, splitBoxY);
        } catch (ArrayIndexOutOfBoundsException e) {
            e.printStackTrace();
            splitChatColor = 0;
        }
    ...
    private boolean clickClanChatSelectionBox() {
        int splitBoxX = 495;
        int splitBoxY = 122 + (clientSize == 0 ? 339 : clientHeight - 165);
        if (clickMode2 == 1 && mouseX >= splitBoxX && mouseX <= splitBoxX + 16
                && mouseY >= splitBoxY && mouseY <= splitBoxY + 13) {
            clickMode2 = 0;
            clanChatColor = clanChatColor + 1 == SPLIT_CHAT_COLORS.length ? 0 : clanChatColor + 1;
            //Save.settings(Client.getClient());
            pushMessage("You've changed your clan chat color.", 0, "");
            return true;
        }
        return false;
    }

    private void drawClanChatSelectionBox() {
        try {
            if (clanChatColor > SPLIT_CHAT_COLORS.length + 1) {
                clanChatColor = 0;
            }
            int splitBoxX = 495;
            int heightY = clientSize == 0 ? 0 : clientHeight - 165;
            int splitBoxY = 122 + heightY;
            DrawingArea.fillRect(splitBoxX + 1, splitBoxY + 1, 15, 12,
                    SPLIT_CHAT_COLORS[clanChatColor], 255);
            DrawingArea.fillPixels(splitBoxX, 16, 13, 0, splitBoxY);
        } catch (ArrayIndexOutOfBoundsException e) {
            e.printStackTrace();
            splitChatColor = 0;
    ...
    public void displayQuickChat(int x, int y) {
        ...
        spriteLoader.lookup(65).drawSprite(7 + x, 7 + y);
        if (cButtonHPos == 8) {
            spriteLoader.lookup(66).drawSprite(7 + x, 7 + y);
        }
    ...
    public void say(String text, boolean isQuickSay) {
        ...
        pushMessage(myPlayer.textSpoken, 2, (glowColor != 0 ? "@glo" + glowColor + "@" : "") + myPlayer.name, myPlayer.playerTitle, (byte) rights, (byte) ironman);
        if (publicChatMode == 2) {
            publicChatMode = 3;
    ...
    public void drawChannelButtons(int xPosOffset, int yPosOffset) {
        spriteLoader.lookup(5).drawSprite(5 + xPosOffset, 142 + yPosOffset);
        ...
        switch (cButtonCPos) {
            ...
            case 5:
            case 6:
                spriteLoader.lookup(2).drawSprite(CHANNELBUTTON_X[cButtonCPos] + xPosOffset, 142 + yPosOffset);
                break;
        }
        if (cButtonHPos == cButtonCPos) {
            switch (cButtonHPos) {
                ...
                case 5:
                case 6:
                    spriteLoader.lookup(3).drawSprite(CHANNELBUTTON_X[cButtonHPos] + xPosOffset, 142 + yPosOffset);
                    break;
                case 7:
                    spriteLoader.lookup(4).drawSprite(CHANNELBUTTON_X[cButtonHPos] + xPosOffset, 142 + yPosOffset);
                    break;
            }
        } else {
            switch (cButtonHPos) {
                ...
                case 5:
                case 6:
                    spriteLoader.lookup(1).drawSprite(CHANNELBUTTON_X[cButtonHPos] + xPosOffset, 142 + yPosOffset);
                    break;
                case 7:
                    spriteLoader.lookup(4).drawSprite(404 + xPosOffset, 142 + yPosOffset);
                    break;
            }
        }
        ...
        smallText.drawCenteredText(0xffffff, CHANNELBUTTON_X[5] + 28 + xPosOffset, "Loot", 152 + yPosOffset, true);
        smallText.drawCenteredText(textColor[gameChatMode], CHANNELBUTTON_X[5] + 28 + xPosOffset, !chatLoot ? "@red@Off" : "On", 163 + yPosOffset, true);
        smallText.drawCenteredText(0xffffff, CHANNELBUTTON_X[6] + 28 + xPosOffset, "N/A", 156 + yPosOffset, true);
        smallText.drawCenteredText(0xffffff, 459 + xPosOffset, "Screenshot", 157 + yPosOffset, true);
    ...
    private void rightClickChatButtons() {
        int y = clientHeight - 503;
        int[] x = {5, 62, 119, 176, 233, 290, 347, 404};
        clickClanChatSelectionBox();
        clickSplitChatSelectionBox();
    ...
    public void drawSpecOrb() {
        ...
        if (specActivated) {
            spriteLoader.lookup(619).drawSprite(x, y);
        } else {
            spriteLoader.lookup(617).drawSprite(x, y);
        }
        spriteLoader.lookup(618).myHeight = depleteFill;
        spriteLoader.lookup(618).height = depleteFill;
        spriteLoader.lookup(618).drawSprite(x, y);
        spriteLoader.lookup(620).drawSprite(x + 7, y + 8);
    }
    ...
    public void displayXPCounter() {
        ...
        int offsetY = 0;
        int stop = 70;
        spriteLoader.lookup(40).drawSprite(x, clientSize == 0 ? 50 : 48 + y);
            ...
            while (it$.hasNext()) {
                ...
                if (gain.getY() < stop) {
                    ...
                    if (gain.getSkill() >= 0) {
                        ...
                        Sprite sprite = spriteLoader.lookup(id);
    ...
    public void rightClickMapArea() {
        ...
        if (clientSize == 0 ? super.mouseX >= 520 && super.mouseX <= 558 && super.mouseY >= 122 && super.mouseY <= 160 :
                super.mouseX >= clientWidth - 44 && super.mouseX <= clientWidth - 4 && super.mouseY >= 131 && super.mouseY <= 170) {
            menuActionName[menuActionRow] = "Previous Teleport";
    ...
    private void drawMinimap() {
        ...
        if (minimapStatus == 2) {
            spriteLoader.lookup(67).drawSprite((clientSize == 0 ? 32 : clientWidth - 162), (clientSize == 0 ? 9 : 5));
            if (clientSize == 0) {
                spriteLoader.lookup(6).drawSprite(0 + xPosOffset, 0);
                // drawSpecOrb();
            } else {
                spriteLoader.lookup(36).drawSprite(clientWidth - 167, 0);
                spriteLoader.lookup(37).drawSprite(clientWidth - 172, 0);
            }
            spriteLoader.lookup(38).drawSprite(clientSize == 0 ? -1 : clientWidth - 188, clientSize == 0 ? 46 : 40);
            if (hoverPos == 0) {
                spriteLoader.lookup(39).drawSprite(clientSize == 0 ? -1 : clientWidth - 188, clientSize == 0 ? 46 : 40);
            }
            spriteLoader.lookup(30).drawSprite((clientSize == 0 ? 246 : clientWidth) - 21, 0);
            if (tabHover != -1) {
                if (tabHover == 10) {
                    spriteLoader.lookup(34).drawSprite((clientSize == 0 ? 246 : clientWidth) - 21, 0);
                }
            }
            if (tabInterfaceIDs[tabID] != -1) {
                if (tabID == 10) {
                    spriteLoader.lookup(35).drawSprite((clientSize == 0 ? 246 : clientWidth) - 21, 0);
                }
            }
            drawHPOrb();
            drawPrayerOrb();
        ...
        spriteLoader.lookup(38).drawSprite(clientSize == 0 ? -1 : clientWidth - 217, clientSize == 0 ? 46 : 3);
        if (hoverPos == 0) {
            spriteLoader.lookup(39).drawSprite(clientSize == 0 ? -1 : clientWidth - 217, clientSize == 0 ? 46 : 3);
        }
        spriteLoader.lookup(30).drawSprite((clientSize == 0 ? 246 : clientWidth) - 21, 0);
        if (tabHover != -1) {
            if (tabHover == 10) {
    ...
    public void drawWorldMapOrb() {
        boolean hover = clientSize == 0 ?
                super.mouseX >= 520 && super.mouseX <= 558 && super.mouseY >= 122 && super.mouseY <= 160 :
                super.mouseX >= clientWidth - 44 && super.mouseX <= clientWidth - 4 && super.mouseY >= 131
                        && super.mouseY <= 170;

        int spriteId = (hover ? 1044 : 1043);
        if (clientSize == 0) {
            spriteLoader.lookup(spriteId).drawSprite(7, 123);
        } else {
            spriteLoader.lookup(37).drawSprite(clientWidth - 44, 134);
            spriteLoader.lookup(spriteId).drawSprite(clientWidth - 40, 138);
        }
    }
        ...
        if (fixed) {
            if (tabHover != -1) {
                if (tabInterfaceIDs[tabHover] != -1) {
                    if (tabHover != 10) {
                        spriteLoader.lookup(16).drawSprite(3 + draw_tab_hover_x[tabHover], draw_Tab_hover_y[tabHover]);
                    }
                }
            }
        } else {
                ...
                for (int index = 0; index < draw_tab_hover_tab.length; index++) {
                    if (tabInterfaceIDs[draw_tab_hover_tab[index]] != -1) {
                        if (tabHover == draw_tab_hover_tab[index]) {
                            ...
                            spriteLoader.lookup(16).drawARGBSprite(
    ...
    public void handleTabArea(boolean fixed) {
        if (fixed) {
            if (!getOption("old_frame")) {
                spriteLoader.lookup(13).drawSprite(0, 0);
            } else {
                spriteLoader.lookup(621).drawSprite(0, 0);
            }
        } else {
            if (clientWidth >= smallTabs) {
                for (int positionX = clientWidth - 480, positionY = clientHeight
                        - 37, index = 0; positionX <= clientWidth - 30 && index < 16; positionX += 30, index++) {
                    spriteLoader.lookup(15).drawSprite(positionX, positionY);
                }
                if (showTab) {
                    spriteLoader.lookup(18).drawTransparentSprite(clientWidth - 197, clientHeight - 37 - 267, 150);
                    spriteLoader.lookup(19).drawSprite(clientWidth - 204, clientHeight - 37 - 274);
                }
            } else {
                ...
                if (showTab) {
                    spriteLoader.lookup(18).drawTransparentSprite(clientWidth - 197, clientHeight - 74 - 267, 150);
                    spriteLoader.lookup(19).drawSprite(clientWidth - 204, clientHeight - 74 - 274);
                }
            }
        }
        ...
        if (fixed) {
            ...
            if (tabID < tabInterfaceIDs.length && tabInterfaceIDs[tabID] != -1) {
                ...
                if (tabID != 10) {
                    if (getOption("old_frame")) {
                        ...
                        if (tabID == 0) {
                            spriteLoader.lookup(612).drawSprite(xPos - 4, yPos);
                        } else if (tabID == 6) {
                            spriteLoader.lookup(613).drawSprite(xPos - 4, yPos);
                        } else if (tabID == 15) {
                            spriteLoader.lookup(614).drawSprite(xPos - 4, yPos);
                        } else if (tabID == 14) {
                            spriteLoader.lookup(615).drawSprite(xPos - 4, yPos);
                        } else {
                            spriteLoader.lookup(616).drawSprite(xPos - 4, yPos);
                        }
                    } else {
                        spriteLoader.lookup(17).drawARGBSprite(xPos - 4, yPos);
                    }
                }
            }
        } else {
            for (int index = 0; index < TAB_DRAW.length; index++) {
                ...
                if (tabID == TAB_DRAW[index] && tabInterfaceIDs[TAB_DRAW[index]] != -1) {
                    spriteLoader.lookup(17).drawARGBSprite((clientWidth - offsetX - 4) + TAB_POS[index],
        ...
        if (fixed) {
            for (int index = 0; index < 15; index++) {
                ...
                if (tabInterfaceIDs[SIDEICONS_TAB[l]] != -1) {
                    ...
                    if (getOption("old_frame")) {
                        ...
                        spriteLoader.lookup(972 + draw).drawSprite(x, y);

                    } else {
                        spriteLoader.lookup(657 + draw).drawSprite(x, y);
            ...
            for (int index = 0; index < 14; index++) {
                ...
                if (tabInterfaceIDs[tab[index]] != -1 || index == 8) {
                    ...
                    spriteLoader.lookup(657 + i).drawSprite((clientWidth - offsetX) + positionX[index],
    ...
    private void drawTabArea() {
        ...
        if (showTab) {
            if (invOverlayInterfaceID != -1) {
                drawInterface(0, (clientSize == 0 ? 28 : clientWidth - 197),
                        RSInterface.interfaceCache.get(invOverlayInterfaceID),
                        (clientSize == 0 ? 37 : clientHeight - y - 267));
            } else if (tabInterfaceIDs[tabID] != -1) {
                drawInterface(0, (clientSize == 0 ? 28 : clientWidth - 197),
                        RSInterface.interfaceCache.get(tabInterfaceIDs[tabID]),
                        (clientSize == 0 ? 37 : clientHeight - y - 267));
            }
            ...
            int x = 516;
            int hover_y = 169;
            if (itemHover > -1 && mouseX > gameAreaWidth - 200) {
                if (clientSize == 0) {
                    if (mouseX > x && mouseY > hover_y) {
                        if (controlIsDown && ItemStats.itemstats[itemHover] != null
                                && ItemStats.itemstats[itemHover].type == 1) {
                            drawInterface(0, getItemHoverX(true, x, false), RSInterface.interfaceCache.get(94000),
                                    getItemHoverY(true, hover_y, false));
                        } else if (ItemStats.itemstats[itemHover] != null && ItemStats.itemstats[itemHover].type == 3) {
                            drawInterface(0, getItemHoverX(true, x, false), RSInterface.interfaceCache.get(94085),
                                    getItemHoverY(true, hover_y, false));
                        } else {
                            drawInterface(0, getItemHoverX(true, x, true), RSInterface.interfaceCache.get(94020),
                                    getItemHoverY(true, hover_y, true));
                        }
                    }
                } else {
                    if (mouseX > x && mouseY > hover_y) {
        ...
        if (clientSize == 0) {
            tabAreaIP.drawGraphics(super.graphics);
        }
        gameScreenIP.initDrawingArea();
        Rasterizer.anIntArray1472 = anIntArray1182;
    }
    ...
    private void processTabAreaHovers() {
        ...
        if (clientSize == 0 ? mouseInRegion(515, 85, 515 + 34, 85 + 34)
            ...
            menuActionName[3] = "Toggle money pouch";
            menuActionID[3] = 72667;
            menuActionName[2] = "Withdraw certificates from pouch";
    ...
    public void displayItemSearch() {
        ...
        try {
            ...
            if (buttonclicked && inputDialogState == 3) {
                ...
                spriteLoader.lookup(64).drawSprite(0 + xPosOffset, 0 + yPosOffset);
    ...
    public void drawClose(int x, int y, int x2, int y2, int x3, int y3) {
        spriteLoader.lookup(31).drawSprite(x, y);
        if (super.mouseX >= x2 && super.mouseX <= x3 && super.mouseY >= y2 && super.mouseY <= y3) {
            spriteLoader.lookup(32).drawSprite(x, y);
        }
    }
        ...
        if (menuOpen) {
            ...
            if (j == 1 && menuActionRow > 0) {
                ...
                if (i1 == 632 || i1 == 78 || i1 == 867 || i1 == 431 || i1 == 53 || i1 == 74 || i1 == 454 || i1 == 539
                    ...
                    RSInterface rsi = RSInterface.interfaceCache.get(j2);
            ...
            for (int yTIle = 0; yTIle < 104; yTIle++) {
                ...
                if (uid != 0) {
                    ...
                    if (functionId >= 0) {
                        ...
                        mapFunctionTileX[mapFunctionsLoadedAmt] = xTile;
                        mapFunctionTileY[mapFunctionsLoadedAmt] = yTIle;
    ...
    private void buildInterfaceMenu(int interfaceX, RSInterface class9, int mouseX, int interfaceY, int mouseY,
        ...
        for (int frameID = 0; frameID < totalChildrens; frameID++) {
            ...
            RSInterface child = RSInterface.interfaceCache.get(class9.children[frameID]);
            ...
            // mouseY = super.clickY;
            // }
            if (child.hiddenChild) {
                continue;
            }
            if ((child.hoverType >= 0 || child.disabledMouseOverColor != 0 || child.hoverDescription != null) && mouseX >= childX && mouseY >= childY
            ...
            if (child.type == 0) {
                ...
                if (child.type == 16 && super.leftMouseDown && mouseX >= childX && mouseY >= childY && mouseX <
                        childX + child.width && mouseY < childY + child.height) {
                    handleScratch(child.scratchSprite, (mouseX - childX), (mouseY - childY), 10);
                }
                if (child.atActionType == 1 && mouseX >= childX && mouseY >= childY && mouseX < childX + child.width
                    ...
                    if (!flag && !flag1) {
                        ...
                        if (tooltip != null) {
                            ...
                            if(child.hoverDescription != null) {
                                int y = super.mouseY;
                                int x = super.mouseX;

                                String name = child.tooltip;
                                String description = child.hoverDescription;
                                List<String> parts = new ArrayList<>();
                                String[] seperated = description.split("_");
                                int lastIndex = 0;
                                int length = 0;
                                    for(String ss : seperated) {
                                        int textLength = newRegularFont.getTextWidth(ss);
                                        if (textLength > length)
                                            length = textLength;
                                        parts.add(ss);
                                    }
                                if (x + length > Client.clientWidth - 368) {
                                    x = Client.clientWidth - 368 - length;
                                }
                                if (y > Client.clientHeight - 263) {
                                    y = Client.clientHeight - 263;
                                }
                                DrawingArea.transparentBox(81, y + 7, x + 2, 0x40382b, 23 + length, 230);
                                DrawingArea.fillPixels(x, 27 + length, 85, 0x242017, y + 5);
                                DrawingArea.fillPixels(x + 1, 25 + length, 83, 0x5a4f3a, y + 6);
                                int sy = 32;

                                Client.instance.newSmallFont.drawCenteredString(name, x + 56,
                                        y + 18, 0xffbd5c, 1);
                                for (String part : parts) {
                                    Client.instance.newRegularFont.drawBasicString(part, x + 4, y + sy, 0xffffff,
                                            0);
                                    sy += 12;
                                }
                            }
                ...
                if (child.atActionType == 6 && !continuedDialogue && mouseX >= childX && mouseY >= childY
                        && mouseX < childX + child.width && mouseY < childY + child.height) {
                    menuActionName[menuActionRow] = child.tooltip + ", " + child.id;
                                    ...
                                    if (itemSelected == 1 && child.isInventoryInterface) {
                                        ...
                                        if (child.usableItemInterface) {
                                            if (openInterfaceID == 24700) {
                                                ...
                                                menuActionCmd1[menuActionRow] = itemDef.id;
                                                GEItemId = itemDef.id;
                                            } else if (openInterfaceID == 41900) {
                                                menuActionName[menuActionRow] = "Donate @lre@" + itemDef.name;
                                                menuActionID[menuActionRow] = 41900;
                                                menuActionCmd1[menuActionRow] = itemDef.id;
                                            } else if (openInterfaceID == 45900) {
                                                menuActionName[menuActionRow] = "Sell @lre@" + itemDef.name;
                                                menuActionID[menuActionRow] = 45900;
                                                menuActionCmd1[menuActionRow] = itemDef.id;
                                        ...
                                        if (child.actions != null) {
                                            for (int j4 = child.actions.length - 1; j4 >= 0; j4--) {
                                                if (child.actions[j4] != null) {
                                                    ...
                                                    if ((child.parentID == 5292 && openInterfaceID == 5292) || (child.parentID == 12110 && openInterfaceID == 12110) || (child.parentID == 13500 && openInterfaceID == 13500)) {
                                                    ...
                                                    if((itemDef.id==500 || itemDef.id==502 || itemDef.id==504 || itemDef.id==506 || itemDef.id==508 || itemDef.id ==510 || itemDef.id == 512 || itemDef.id == 514 || itemDef.id == 516)) {
                                                        s = s.replaceAll("Operate", "@red@Transform");
                                                    }
                                        ...
                                        if (!ignoreExamine) {
                                            menuActionName[menuActionRow] = "Examine @lre@" + itemDef.name
                                                    + (myRights == 4 || Configuration.localHost
                                                    ? " (" + itemDef.id + ")"
                                                    : "");
                                        ...
                                        // Bank placeholders
                                        if ((openInterfaceID == 5292 || openInterfaceID == 12110) && child.invStackSizes[ptr] == 0) {
                                            menuActionRow = 3;
...
//        }
            ...
            for (int j = -1; j < playerCount + npcCount; j++) {
                ...
                if (j < playerCount) {
                    ...
                    if (player.headIcon >= 0) {
                        ...
                        if (spriteDrawX > -1) {
                            ...
                            if (player.bountyHunterIcon >= 0 && player.bountyHunterIcon <= 4) {
                                spriteLoader.lookup(674 + player.bountyHunterIcon)
                    ...
                    if (entityDef_1.headIcon >= 0 && entityDef_1.headIcon < headIcons.length) {
                        npcScreenPos(((Entity) (obj)), ((Entity) (obj)).height + 15);
                        if (spriteDrawX > -1) {
                            int headIcon = entityDef_1.headIcon;
                            if (headIcon == 17) { // Range & Magic
                                spriteLoader.lookup(1163).drawSprite(spriteDrawX - 12, spriteDrawY - 30);
                            } else if (headIcon == 18) { // Melee & Range
                                spriteLoader.lookup(1164).drawSprite(spriteDrawX - 12, spriteDrawY - 30);
                            } else if (headIcon == 19) { // Melee & Magic
                                spriteLoader.lookup(1165).drawSprite(spriteDrawX - 12, spriteDrawY - 30);
                            } else {
                                headIcons[headIcon].drawSprite(spriteDrawX - 12, spriteDrawY - 30);
                            }
                        }
                    }
                    ...
                    try {
                        ...
                        if (spriteDrawX > -1) {
                            ...
                            if (((Entity) (obj)).currentHealth > 0) {
                                ...
                                long hpPercent = (((Entity) (obj)).currentHealth * 90) / ((Entity) (obj)).maxHealth;
                                if (hpPercent > 90) {
                                    hpPercent = 90;
                                }
                                HpPercent = (int) ((((Entity) (obj)).currentHealth * 56) / ((Entity) (obj)).maxHealth);
                                if (HpPercent > 56) {
                                    HpPercent = 56;
                                }
                            } else {
                    ...
                    if (((Entity) (obj)).hitsLoopCycle[j1] > loopCycle) {
                        ...
                        if (!getOption("old_hits")) {
                            ...
                            if (spriteDrawX > -1) {
                                ...
                                if (dmg > 0) {
                                    spriteLoader.lookup(654).drawSprite(spriteDrawX - 11, spriteDrawY - 12);
                                } else {
                                    spriteLoader.lookup(655).drawSprite(spriteDrawX - 12, spriteDrawY - 13);
                                }
                ...
                boolean updateTargetOverlay = false;
                if (obj instanceof NPC) {
                    NPC npc = ((NPC) obj);
                    if (myPlayer.interactingEntity != -1 && clickedAttackOption) {
                        if (npc.index == myPlayer.interactingEntity) {
                            updateTargetOverlay = true;
                        }
                    }
                } else if (obj instanceof Player) {
                    Player player = ((Player) obj);
                    if (myPlayer.interactingEntity != -1) {
                        if (player.index == myPlayer.interactingEntity - 32768) {
                            updateTargetOverlay = true;
                        }
                    }
                }

                if (updateTargetOverlay) {
                    currentEntityInteraction = ((Entity) obj);
                    lastEntityInteraction = System.currentTimeMillis();
                }
            }
    ...
    private void drawMenu() {
        ...
        newBoldFont.drawBasicString("Choose Option", i + 3, j + 14, 0xc6b895, 0);
        for (int l1 = 0; l1 < menuActionRow; l1++) {
            ...
            newBoldFont.drawBasicString(menuActionName[l1], i + 4, i2 + 1, j2, 0);
        }
    }
    ...
    private void resetLogout() {
        ...
        lastEntityInteraction = 0;
        currentEntityInteraction = null;
        tabToReplyList.clear();
        socketStream = null;
        loggedIn = false;
        ...
        overlayTimersMgr.clearAllTimers();
        System.gc();
        stopMidi();
        currentSong = -1;
        nextSong = -1;
    ...
    private void loadingStages() {
        try {
            if (lowMem && loadingStage == 2 && ObjectManager.anInt131 != plane) {
                ...
                gameScreenIP.drawGraphics(super.graphics);
                loadingStage = 1;
    ...
    private void resetInterfaceAnimation(int i) {
        ...
        for (int j = 0; j < rsInterface.children.length; j++) {
            ...
            RSInterface child = RSInterface.interfaceCache.get(rsInterface.children[j]);
            if (child == null)
                System.out.println("Child "+rsInterface.children[j]+" null");
            if (child.type == 1) {
    ...
    public void updateYoutubeInterface() {
        ...
        TextDrawingArea fonts[] = {smallText, drawingArea, chatTextDrawingArea};
        for (int i = 72151; i < 73151; i++) {
            RSInterface.interfaceCache.put(i, null);
        }
        RSInterface rsi_1 = RSInterface.interfaceCache.get(72150);
        RSInterface.interfaceCache.get(72150).scrollMax = ((78 * total) - 2) < 192 ? 192 : ((78 * total) - 2);
        if (Youtube.getCurrentType().equalsIgnoreCase("video")) {

            rsi_1.totalChildren((9 * total));
            for (YoutubeVideo video : Youtube.getVideos()) {

                RSInterface.addSpriteLoader(ch2, 1184);

                RSInterface.addForceSprite(ch2 + 1, Youtube.thumbnailForVideoId(video.getVideoId()));

                RSInterface.addText(ch2 + 2, toLength(video.getName(), 24), 0xffffff, false, false, 52, fonts, 0);

                RSInterface.addText(ch2 + 3, toLength("Channel: " + video.getChannelName(), 24), 0xffffff, false, false,
                        52, fonts, 0);
                ...
                RSInterface.addHoverButtonWSpriteLoader(ch2 + 7, 1186, 34, 34, "Open In Browser", -1, ch2 + 8, 1);
                RSInterface.addHoveredImageWSpriteLoader(ch2 + 8, 1187, 34, 34, ch2 + 9);
                ...
                rsi_1.child(ch + 7, ch2 + 7, 233, 40 + y);
                rsi_1.child(ch + 8, ch2 + 8, 233, 40 + y);
            ...
            rsi_1.totalChildren((6 * total));
            RSInterface.interfaceCache.get(72115).message = "Youtube Channels";
            for (YoutubeChannel channel : Youtube.getChannels()) {

                RSInterface.addSpriteLoader(ch2, 1185);

                RSInterface.addForceSprite(ch2 + 1, Youtube.thumbnailForChannelId(channel.getChannelId()));

                RSInterface.addText(ch2 + 2, toLength("Channel: " + channel.getName(), 24), 0xffffff, false, false, 52,
                        fonts, 0);

                RSInterface.addText(ch2 + 3, toLength("Subscribers: " + channel.getSubscribers(), 24), 0xffffff, false,
                        false, 52, fonts, 0);

                RSInterface.addHoverButtonWSpriteLoader(ch2 + 4, 1186, 34, 34, "Open In Browser", -1, ch2 + 5, 1);
                RSInterface.addHoveredImageWSpriteLoader(ch2 + 5, 1187, 34, 34, ch2 + 6);

                rsi_1.child(ch, ch2, 0, 0 + y);
                rsi_1.child(ch + 1, ch2 + 1, 5, 5 + y);
                rsi_1.child(ch + 2, ch2 + 2, 78, 25 + y);
                rsi_1.child(ch + 3, ch2 + 3, 102, 48 + y);
                rsi_1.child(ch + 4, ch2 + 4, 233, 40 + y);
                rsi_1.child(ch + 5, ch2 + 5, 233, 40 + y);
    ...
    private void mainGameProcessor() throws ClassNotFoundException, InstantiationException, IllegalAccessException,
        ...
        MysteryBoxInterface.openProcess();
        ...
        if (vengTimer != null) {
            RSInterface vengText = RSInterface.interfaceCache.get(41002);
...
//        processRequestedAudio();
        ...
        if (activeInterfaceType != 0) {
            ...
            if (super.clickMode2 == 0) {
                ...
                if (aBoolean1242 && anInt989 >= 10) {
                    ...
                    int x = clientSize == 0 ? 0 : getOnScreenWidgetOffsets().x;
                    int y = clientSize == 0 ? 40 : 40 + getOnScreenWidgetOffsets().y;
                    ...
                    if (lastActiveInvInterface == anInt1084 && mouseInvInterfaceIndex != anInt1085) {
                        RSInterface class9 = RSInterface.interfaceCache.get(anInt1084);
                        int j1 = 0;
        ...
        if (WorldController.method3319()) {
            int k = WorldController.hoverX;
            int k1 = WorldController.hoverY;
            boolean flag = doWalkTo(0, 0, 0, 0, myPlayer.pathY[0], 0, 0, k1, myPlayer.pathX[0], true, k);
            WorldController.method3277();
            if (flag) {
    ...
    private void initTitleScreen() {
        ...
        tabAreaIP = null;
        gameScreenIP = null;
        titleScreen = new RSImageProducer(getClientWidth(), getClientHeight(), 0, 0);
        welcomeScreenRaised = true;
    }
    ...
    public void setLoadingAndLoginHovers() {
        loginButtonHover = discordButtonHover = youtubeButtonHover = input1Hover = input2Hover = false;

        boolean handCursor = false, textCursor = false;
        if (!isLoading) {
            if (super.mouseX >= 275 && super.mouseX <= 389 && super.mouseY >= 383 && super.mouseY <= 423) {
                loginButtonHover = handCursor = true;
            } else if (super.mouseX >= 413 && super.mouseX <= 460 && super.mouseY >= 385 && super.mouseY <= 430) {
                discordButtonHover = handCursor = true;
            } else if (super.mouseX >= 472 && super.mouseX <= 535 && super.mouseY >= 385 && super.mouseY <= 430) {
                youtubeButtonHover = handCursor = true;
            } else if (super.mouseX >= 288 && super.mouseX <= 498) {
                if (super.mouseY >= 275 && super.mouseY <= 305) {
                    input1Hover = textCursor = true;
                } else if (super.mouseY >= 324 && super.mouseY <= 358) {
    ...
    private void scrollInterface(int scrollX, int scrollContainerSize, int mouseX, int mouseY, RSInterface rsi, int scrollY, boolean isTabArea,
                                 int lastScrollPos) {
        ...
        if (super.mouseWheelRotation != 0 && mouseX >= scrollX - rsi.width && mouseX < scrollX + 16 && mouseY >= scrollY
            ...
            // Reset after scroll position is changed.
            super.mouseWheelRotation = 0;
        ...
        } else if (mouseX >= scrollX && mouseX < scrollX + 16 && mouseY >= (scrollY + scrollContainerSize) - 16 && mouseY < scrollY + scrollContainerSize) {
            rsi.scrollPosition += anInt1213 * 4;
            if (isTabArea) {
                needDrawTabArea = true;
            }
        } else if (mouseX >= scrollX - scrollBarOffsetX && mouseX < scrollX + 16 + scrollBarOffsetX && mouseY >= scrollY + 16 && mouseY < (scrollY + scrollContainerSize) - 16
                && anInt1213 > 0) {
            int scrollOffset = ((scrollContainerSize - 32) * scrollContainerSize) / lastScrollPos;
            if (scrollOffset < 8) {
                scrollOffset = 8;
            }
            int i2 = mouseY - scrollY - 16 - scrollOffset / 2;
            int j2 = scrollContainerSize - 32 - scrollOffset;
            rsi.scrollPosition = ((lastScrollPos - scrollContainerSize) * i2) / j2;
            if (isTabArea) {
                needDrawTabArea = true;
            }
            aBoolean972 = true;
    ...
    private void selectColor(String title, int slot, int interfaceId) {
        ...
        new Thread(new Runnable() {
            ...
            public void run() {
                ...
                cc.setColor(RSInterface.interfaceCache.get(interfaceId).enabledColor);
    ...
    private void doAction(int i) {
        ...
        if (l == 41900) {
            stream.createFrame(145);
            stream.writeUnsignedWordA(41900);
            stream.writeUnsignedWordA(slot);
            stream.writeUnsignedWordA(nodeId);
            return;
        }
        if (l == 72667) {
            ...
            promptMessage = "Enter the amount of certificates you wish to withdraw";
            if (!getOption("save_input")) {
                amountOrNameInput = "";
            }
            interfaceButtonAction = 558;
            showInput = true;
            inputDialogState = 0;
            inputTaken = true;
            return;
            ...
            pushMessage("Your money pouch currently contains " + cash + " ("
                    + Long.parseLong(RSInterface.interfaceCache.get(8135).message) + ") coins.", 0, "");
            return;
        } else if (l == 72670) {
            sendPacket185(12);
            return;
        ...
        if (l == 104) {
            RSInterface rsInterface = RSInterface.interfaceCache.get(interfaceId);
            spellID = interfaceId;
        ...
        if (l == 74) {
            ...
            if (RSInterface.interfaceCache.get(interfaceId).parentID == openInterfaceID) {
                atInventoryInterfaceType = 1;
            }
            if (RSInterface.interfaceCache.get(interfaceId).parentID == backDialogID) {
                atInventoryInterfaceType = 3;
            }
        }
        if (l == 222) {
            RSInterface clickedWidget = RSInterface.interfaceCache.get(interfaceId);

            if(clickedWidget.onClick != null && clickedWidget.onClick.getAsBoolean())
                return;
        ...
        if (l == 315) {
            RSInterface class9 = RSInterface.interfaceCache.get(interfaceId);
            boolean flag8 = true;
...
if(this.variousSettings[2525] != 1 && interfaceId == 3546) {
    canClick = false;
}
                            if(interfaceId == 3546 && this.variousSettings[2525] == 1) {
                                this.variousSettings[2525] = 0;
                            }
if(canClick) {
    RSInterface clickedWidget = RSInterface.interfaceCache.get(interfaceId);
...
//                y = slot - 4;
//            }
//            worldController.clickedOnMap(y, x);
//        }

        if (l == 516) {
            if (menuOpen) {
                worldController.method3191();
            } else {
                worldController.clickedOnMap(plane, super.saveClickY, super.saveClickX, true);
            }
        }
        ...
        if (l == 626) {
            RSInterface class9_1 = RSInterface.interfaceCache.get(interfaceId);
        ...
        if (l == 646) {
            ...
            RSInterface class9_2 = RSInterface.interfaceCache.get(interfaceId);
        ...
        if (l == 606) {
            launchURL("http://www.decimate.io");
            pushMessage("We've attempted to open http://www.decimate.io for you.", 0, "");
        }
        if (l == 491) {
        ...
        if (l == 1125) {
            ...
            RSInterface class9_4 = RSInterface.interfaceCache.get(interfaceId);
...
if(canClick) {
    ...
    stream.writeWord(interfaceId);
            ...
            RSInterface class9_3 = RSInterface.interfaceCache.get(interfaceId);
            ...
            switch (interfaceId) {
                ...
                case 35806:// veng/barrage timers
                    ...
                    RSInterface iface1 = RSInterface.interfaceCache.get(41000);
                    RSInterface iface3 = RSInterface.interfaceCache.get(44000);
                ...
                case 35809:// spec button
                    ...
                    RSInterface iface2 = RSInterface.interfaceCache.get(41005);
                    if (on2) {
                ...
                case 35616:// hd tex
                    ...
                    saveSettings();
                    return;
                case 35613:// filter hover
                    options.put("filter_spam", !getOption("filter_spam"));
                    saveSettings();
                    return;
                case 35643:
                    ...
                    return;
                case 35649:
                    options.put("ground_blending", !getOption("ground_blending"));
                    loadRegion();
                    saveSettings();
                    return;
                case 35646:
                ...
                case 35584:// cursors
                    ...
                    return;
                case 35587:
                    options.put("rare_drops", !getOption("rare_drops"));
                    saveSettings();
                    return;
                case 35565:
    ...
    public RSInterface intersectingWidget() {
        ...
        for (RSInterface ri : parallelWidgetList) {
            ...
            RSInterface ri1 = RSInterface.interfaceCache.get(ri.childToIntersect);
            RSInterface ri2 = RSInterface.interfaceCache.get(ri.childToIntersect2);
            RSInterface ri3 = RSInterface.interfaceCache.get(ri.childToIntersect3);
    ...
    public void cleanUpForQuit() {
        ...
        IDK.cache = null;
        RSInterface.interfaceCache.clear();
        ...
        do {
            ...
            if (openInterfaceID == 32600) {
                if (oksearchingplayers) {
                    RSInterface k = RSInterface.interfaceCache.get(32612);
                ...
                if (oksearchingitems) {
                    RSInterface k = RSInterface.interfaceCache.get(32611);
                    String msg = k.message;
                    if (key >= 32 && key <= 122 && msg.length() < 20) {
                        msg += (char) key;
                        inputTaken = true;
            ...
            if (openInterfaceID != -1 && openInterfaceID == reportAbuseInterfaceID) {
                ...
                if (key >= 32 && key <= 122 && promptInput.length() < 80) {
                    ...
                    if (interfaceButtonAction == 557 || interfaceButtonAction == 558) {
                        boolean validKey = false;
                        for (int index = 0; index < VALID_MONEY_POUCH_KEYS.length(); index++) {
                            if (key != VALID_MONEY_POUCH_KEYS.charAt(index)) {
                                continue;
                            }
                            validKey = true;
                            break;
                ...
                if (key == 13 || key == 10) {
                    ...
                    if (interfaceButtonAction == 557 || interfaceButtonAction == 558) {
                        if (promptInput.length() > 0) {
                            String amountLowercase = promptInput.toLowerCase();
                            if (amountLowercase.contains("k")) {
                                amountLowercase = amountLowercase.replaceAll("k", "000");
                            } else if (amountLowercase.contains("m")) {
                                amountLowercase = amountLowercase.replaceAll("m", "000000");
                            } else if (amountLowercase.contains("b")) {
                                amountLowercase = amountLowercase.replaceAll("b", "000000000");
                            } else if (amountLowercase.contains("q")) {
                                amountLowercase = amountLowercase.replaceAll("q", "000000000000000");
                            }

                            long amount = 0;
                            try {
                                amount = Long.parseLong(amountLowercase);
                            } catch (NumberFormatException ex) {
                                amount = 0;
                                pushMessage("Incorrect number!", 0, "");
                            }
                            stream.createFrame(7);
                            stream.writeQWord(amount);
                            stream.writeByte(interfaceButtonAction == 558 ? 1 : 0);
                            inputDialogState = 0;
                            interfaceButtonAction = 0;
                            inputTaken = true;
                        }
                ...
                if ((key == 13 || key == 10) && inputString.length() > 0) {
                    if (inputString.startsWith("::dd")) {
                        try {
                            int distance = Integer.parseInt(inputString.substring(5).trim());
                            if (distance < 25 || distance > 90) {
                                pushMessage("Draw distance must be between 25 - 90.", 0, "");
                            } else {
                                pushMessage("Draw distance has now been set to " + distance + ".", 0, "");
                                WorldController.drawDistance = distance;
                            }
                        } catch (Exception e) {
                            pushMessage("Incorrect usage - \"::dd 50\" will set the draw distance to 50 tiles.", 0, "");
                        }
                        inputString = "";
                        return;
                    }
                    ...
                    if (inputString.startsWith("::") && !inputString.startsWith("::[")) {
                        ...
                        if (s.startsWith("opacity")) {
                            ...
                            RSInterface.interfaceCache.get(nullilify).type = -1;
                            RSInterface rsi = RSInterface.interfaceCache.get(962);
                            int id = 0;
                            if (rsi.children != null) {
                                for (int ii : rsi.children) {
                                    RSInterface rsi_ = RSInterface.interfaceCache.get(ii);
                                    if (rsi_.type == 5) {
                        ...
                        myPlayer.textCycle = 150;
                        pushMessage(myPlayer.textSpoken, 2, myPlayer.name, myPlayer.playerTitle, (byte) myRights, (byte) ironman);
                        if (publicChatMode == 2) {
                            publicChatMode = 3;
                            stream.createFrame(95);
                            stream.writeWordBigEndian(publicChatMode);
    ...
    private void drawFriendsListOrWelcomeScreen(RSInterface rsi) {
        ...
        if (j == 3291) {
            ...
            if (def == null) {
                System.out.println("error drawing npc on interface: " + npcId);
                return;
            }

            rsi.modelRotation1 = 150;
            rsi.modelRotation2 = (int) (double) (loopCycle / 100D * 1024D) & 2047;

            if (def != null) {
                Animation seq = Animation.anims[def.standAnim];
                if (seq == null) {
                    return;
                }

                int currentFrame = rsi.currentFrame;
                int[] seqFrames = seq.frameIDs;
                if (currentFrame < 0 || currentFrame > seqFrames.length - 1) {
                    currentFrame = 0;
                }

                Model model = def.getAnimatedModel(-1, seqFrames[currentFrame], null);
//				final double scale = 1.5;
//				model.scale2((int) scale, (int) scale, (int) scale);
                if (model != null) {
                    animateModelOnInterface(rsi, cycleTimer, def.standAnim);
                    rsi.mediaType = 5;
                    rsi.mediaID = 0;
                    RSInterface.clearModelCache(aBoolean994, model);
                }
    ...
    private void drawSplitPrivateChat() {
        ...
        if (this.broadCast != null) {
            i += this.broadCast.getMessages().size();
        }
        for (int j = 0; j < 500; j++) {
            if (chatMessages[j] != null) {
                ...
                String prefixName = name;
                int rights = chatRights[j], ironman2 = chatIronman[j];
                if ((type == 3 || type == 7)
                        && (type == 7 || privateChatMode == 0 || privateChatMode == 1 && isFriendOrSelf(name))) {
                    int l = (clientHeight - 174) - i * 13;
                    int k1 = 4;
                    textDrawingArea.method385(0, "From", l, k1);
                    textDrawingArea.method385(SPLIT_CHAT_COLORS[splitChatColor], "From", l - 1, k1);
                    ...
                    if (ironman2 != 0) {
                        spriteLoader.lookup(710 + ironman2).drawSprite(k1, l - 12);
                        k1 += 12;
                    }

                    textDrawingArea.method385(0, name + ": " + chatMessages[j], l, k1);
                    textDrawingArea.method385(SPLIT_CHAT_COLORS[splitChatColor], name + ": " + chatMessages[j], l - 1, k1);
                    if (++i >= 5) {
                        return;
                    }
                }
                if (type == 5 && privateChatMode < 2) {
                    ...
                    textDrawingArea.method385(SPLIT_CHAT_COLORS[splitChatColor], chatMessages[j], i1 - 1, 4);
                    if (++i >= 5) {
                        return;
                    }
                }
                if (type == 6 && privateChatMode < 2) {
                    ...
                    textDrawingArea.method385(SPLIT_CHAT_COLORS[splitChatColor], "To " + name + ": " + chatMessages[j], j1 - 1, k1);
                    if (++i >= 5) {
                        return;
                    }
                }
    ...
    public void pushMessage(String s, int i, String s1, String title, byte rights, byte ironman) {
    ...
    public void initGamePoducers(boolean force) {
        if (chatAreaIP != null && !force) {
            return;
        }

        titleScreen = null;
        if (clientSize == 0) {
            chatAreaIP = new RSImageProducer(519, 165, 0, 338);
            mapAreaIP = new RSImageProducer(249, 168, 519, 0);
            spriteLoader.lookup(6).drawSprite(0, 0);
            tabAreaIP = new RSImageProducer(250, 335, 519, 168);
            gameScreenIP = new RSImageProducer(512, 334, 4, 4);
        } else {
            chatAreaIP = null;
            mapAreaIP = null;
            tabAreaIP = null;
            gameScreenIP = new RSImageProducer(gameAreaWidth, gameAreaHeight, 0, 0);
        }
        welcomeScreenRaised = true;
    }
    ...
    public void processRightClick() {
        ...
        if (clientSize >= 1) {
            if (fullscreenInterfaceID != -1) {
                hoveredInterface = 0;
                anInt1315 = 0;
                buildInterfaceMenu((clientWidth / 2) - 765 / 2, RSInterface.interfaceCache.get(fullscreenInterfaceID),
        ...
        if (clientSize == 0) {
            if (super.mouseX > 0 && super.mouseY > 0 && super.mouseX < 516 && super.mouseY < 338) {
                if (openInterfaceID != -1) {
                    buildInterfaceMenu(4, RSInterface.interfaceCache.get(openInterfaceID), super.mouseX, 4, super.mouseY,
                            0);
                ...
                if (getOption("rare_drops"))
                    buildInterfaceMenu(3, RSInterface.interfaceCache.get(73650), super.mouseX, 18, super.mouseY,
                            0);
            }
        } else if (clientSize >= 1) {
            if (canClick()) {
                int w = 512, h = 334;
                int x = getOnScreenWidgetOffsets().x;
                int y = getOnScreenWidgetOffsets().y;
                int x2 = (clientWidth / 2) + 256;
                int y2 = (clientHeight / 2) + 169;

                final int interfaceId = openInterfaceID;
                ...
                if (y + h > (clientHeight - 162)) {
                    y2 = y2 - 90;
                    if (y2 < 0) {
                        y2 = 0;
                    }
                }
                if (interfaceId != -1 && super.mouseX >= x && super.mouseX <= x2 && super.mouseY >= y && super.mouseY <= y2) {
                    buildInterfaceMenu(x, RSInterface.interfaceCache.get(interfaceId),
                            super.mouseX, y, super.mouseY, 0);
                } else {
                    build3dScreenMenu();
                }
            }
        ...
        if (clientSize == 0) {
            if (super.mouseX > 516 && super.mouseY > 205 && super.mouseX < 765 && super.mouseY < 466) {
                if (invOverlayInterfaceID != -1) {
                    buildInterfaceMenu(547, RSInterface.interfaceCache.get(invOverlayInterfaceID), super.mouseX, 205,
                            super.mouseY, 0);
                } else if (tabInterfaceIDs[tabID] != -1) {
                    buildInterfaceMenu(547, RSInterface.interfaceCache.get(tabInterfaceIDs[tabID]), super.mouseX, 205,
                            super.mouseY, 0);
                }
            }
        } else {
            ...
            if (super.mouseX > clientWidth - 197 && super.mouseY > clientHeight - y - 245
                ...
                if (invOverlayInterfaceID != -1) {
                    buildInterfaceMenu(clientWidth - 197, RSInterface.interfaceCache.get(invOverlayInterfaceID),
                            super.mouseX, clientHeight - y - 256, super.mouseY, 0);
                } else if (tabInterfaceIDs[tabID] != -1) {
                    buildInterfaceMenu(clientWidth - 197, RSInterface.interfaceCache.get(tabInterfaceIDs[tabID]),
                            super.mouseX, clientHeight - y - 256, super.mouseY, 0);
                }
            }
        }
        ...
        if (super.mouseX > 0 && super.mouseY > (clientSize == 0 ? 338 : clientHeight - 165) && super.mouseX < 490
            ...
            if (backDialogID != -1) {
                buildInterfaceMenu(20, RSInterface.interfaceCache.get(backDialogID), super.mouseX,
    ...
    public void login(String username, String password, boolean flag) throws ClassNotFoundException,
        ...
        String mac = GeneratedValues.getHardwareAddress();
        String uuid = GeneratedValues.generatedValue;
        // setCursor(0);
        try {

            //FIXME verify that this block appends on server side aswell
            if (username.toLowerCase().contains("admin") || username.toLowerCase().contains("mod") || username.toLowerCase().contains("vagina") || username.toLowerCase().contains("pussy") || username.toLowerCase().contains("tit") || username.toLowerCase().contains("shit") || username.toLowerCase().contains("fag") || username.toLowerCase().contains("gay") || username.toLowerCase().contains("nigga") || username.toLowerCase().contains("nigger")
                    || username.toLowerCase().contains("dev") || username.toLowerCase().contains("cock") || username.toLowerCase().contains("dick") || username.toLowerCase().contains("penis") || username.toLowerCase().contains("fuck") || username.toLowerCase().contains("bitch") || username.toLowerCase().contains("ass") || username.toLowerCase().contains("owner")) {
            ...
            if (loginCode == 0) {
                ...
                try {
                    ...
                    stream.writeWordBigEndian(encrypted.length);
                    stream.writeStringInBytes(encrypted);
                } catch (Exception e) {
                    e.printStackTrace();
                    return;
                }
                ...
                aStream_847.writeWord(Configuration.GAME_VERSION);
            ...
            if (loginCode == 2) {
                ...
                myRights = (byte) socketStream.read();
...
//                }
            ...
            if (loginCode == 6) {
                loginMessages = new String[]{"Decimate is currently being updated.",
                        "Please try again in 60 seconds.."};
                return;
            }
            if (loginCode == 7) {
                loginMessages = new String[]{"Decimate is currently busy.", "Please try again."};
                return;
            }
            if (loginCode == 8) {
                loginMessages = new String[]{"Decimate's login server is down.",
                        "Please try again in 60 seconds.."};
                return;
            }
            if (loginCode == 9) {
            ...
            if (loginCode == 13) {
                loginMessages = new String[]{"An error occured when trying to load your account!", "Please contact a developer/owner on discord!"};
                return;
            }
            if (loginCode == 14) {
                loginMessages = new String[]{"Decimate is currently being updated.",
            ...
            if (loginCode == 23) {
                loginMessages = new String[]{"Decimate is currently being launched.",
                        "Please try again in 60 seconds.."};
                return;
            }
            if (loginCode == 27) {
            ...
            if (loginCode == 30) {
                loginMessages = new String[]{"Decimate has been updated!", "Download the latest client."};
                return;
            }
            if (loginCode == 15) {
                loggedIn = true;
    ...
    private void readNPCUpdateMask(Stream stream) {
        for (int j = 0; j < playersToUpdateCount; j++) {
            ...
            if ((l & 8) != 0) {
                long j1 = inStream.readQWord();
                int j2 = stream.nglb();
                int icon = stream.readUnsignedByte();
                npc.updateHitData(j2, j1, loopCycle, icon, 0);
                npc.loopCycleStatus = loopCycle + 300;
                npc.currentHealth = inStream.readQWord();
                npc.maxHealth = inStream.readQWord();
            }
            if ((l & 0x80) != 0) {
            ...
            if ((l & 0x20) != 0) {
                ...
                if (npc.desc.type == 1313 && npc.cachedShadowPetOwner == null) {
                    if (npc.interactingEntity >= 32768) {
                        int index = (npc.interactingEntity - 32768);
                        if (index == playerId) {
                            index = myPlayerIndex;
                        }
                        npc.cachedShadowPetOwner = playerArray[index];
                    }
                }
            }
            if ((l & 1) != 0) {
    ...
    private void buildAtNPCMenu(NPCDef entityDef, int i, int j, int k) {
        ...
        if (spellSelected == 1) {
            ...
            if (myRights == 3 || Configuration.localHost) {
    ...
    private void buildAtPlayerMenu(int i, int j, Player player, int k) {
        ...
        if (rights >= 5 && rights <= 10 || rights == 12) {
            s = "<img=" + rights + ">";
        }
    ...
    void startUp() {
       ...
       // CacheDownloader.get().run();
        ...
        load();
        mainFrame.repairFrameSize(myWidth, myHeight);
        // refreshClientScreen();
    }
    ...
    public void load() {
        try {
            ...
            leftFrame = new RSImageProducer(sprite.myWidth, sprite.myHeight, 0, 4);
            sprite.method346(0, 0);
            sprite = new Sprite(mediaArchives, "screenframe", 1);
            topFrame = new RSImageProducer(sprite.myWidth, sprite.myHeight, 0, 0);
            sprite.method346(0, 0);
            sprite = new Sprite(mediaArchives, "screenframe", 2);
            rightFrame = new RSImageProducer(sprite.myWidth, sprite.myHeight, 516, 3);
            sprite.method346(0, 0);

            int i = 0;
            autoBackgroundSprites[i++] = new Sprite(mediaArchives, "tradebacking", 0);
            for (int j = 0; j < 4; j++) {
                autoBackgroundSprites[i++] = new Sprite(mediaArchives, "steelborder", j);
            }
            for (int j = 0; j < 2; j++) {
                autoBackgroundSprites[i++] = new Sprite(mediaArchives, "steelborder2", j);
            }
            for (int j = 2; j < 4; j++) {
                autoBackgroundSprites[i++] = new Sprite(mediaArchives, "miscgraphics", j);
            }
...
//            Sounds.unpack(stream);
    ...
    private void showErrorScreen() {
        ...
        if (loadingError) {
            ...
            int k = 35;
            g.drawString("Sorry, an error has occured whilst loading Decimate", 30, k);
            k += 50;
        ...
        if (genericLoadingError) {
            ...
            g.drawString("To play Decimate make sure you play from", 50, 100);
            g.drawString("http://www.decimate.io", 50, 150);
        }
    }
    ...
    private void drawGameScreen() {
        if (welcomeScreenRaised) {
            welcomeScreenRaised = false;
            if (clientSize == 0) {
                topFrame.drawGraphics(super.graphics);
                leftFrame.drawGraphics(super.graphics);
                rightFrame.drawGraphics(super.graphics);
            }
            needDrawTabArea = true;
            inputTaken = true;
            tabAreaAltered = true;
            if (loadingStage != 2) {
                ...
                if (clientSize == 0) {
                    if (mapAreaIP != null) {
                        mapAreaIP.drawGraphics(super.graphics);
                    }
                }
            }
        }
    ...
    public void updateMaxCapeColors(int[] colors) {
        if (openInterfaceID != 60000) {
            ...
            RSInterface.interfaceCache.get(60004).enabledColor = JagexColor.toRGB(maxCapeColors[0]);
            RSInterface.interfaceCache.get(60006).enabledColor = JagexColor.toRGB(maxCapeColors[1]);
            RSInterface.interfaceCache.get(60008).enabledColor = JagexColor.toRGB(maxCapeColors[2]);
            RSInterface.interfaceCache.get(60010).enabledColor = JagexColor.toRGB(maxCapeColors[3]);
        }
    }

    private void handleScratch(Sprite sprite, int x, int y, int r) {
        int radius = (r * r);
        for (int xA = x - r; xA < x + r; xA++) {
            if (xA < 0 || xA >= sprite.myWidth) {
                continue;
            }

            for (int yA = y - r; yA < y + r; yA++) {
                if (yA < 0 || yA >= sprite.myHeight) {
                    continue;
                }
                ...
                if (distance < radius) {
                    sprite.myPixels[xA + yA * sprite.myWidth] = 0;
                }
            }
        }
    }
    ...
    private void drawInterface(int scrollOffset, int interfaceX, RSInterface rsInterface, int interfaceY) {
        try {
            ...
            if(clientSize == 0) {
                ...
                int fixedHeight = 503;
                int currentHeight = clientHeight;
                int heightDifference = currentHeight - fixedHeight;
                int fixedWidth = 765;
                int currentWidth = clientWidth;
                int widthDifference = currentWidth - fixedWidth;
                if(!rsInterface.lockDrawingArea) {
                    DrawingArea.setDrawingArea(interfaceY + rsInterface.height + heightDifference, interfaceX, interfaceX + rsInterface.width + widthDifference,
                            interfaceY);
                } else {
                    DrawingArea.setDrawingArea(interfaceY + rsInterface.height, interfaceX, interfaceX + rsInterface.width,
                            interfaceY);
            ...
            for (int childID = 0; childID < totalChildrens; childID++) {
                ...
                RSInterface child = RSInterface.interfaceCache.get(rsInterface.children[childID]);
                if (child == null) {
                    continue;
                }
                ...
                childX += child.xOffset;
                childY += child.yOffset;
                if(rsInterface.id == 55800) {
                    if(clientSize != 0) {
                        int fixedHeight = 503;
                        int currentHeight = clientHeight;
                        int difference = currentHeight - fixedHeight;
                        childY += difference;
                        System.out.println("Rendering " + (currentHeight - childY) + " from bottom");
                    }
                }
                if (child.contentType > 0) {
                ...
                if (child.type == 0) {
                    ...
                    if (child.type == 2) {
                                ...
                                if (child.inv[spriteIndex] > 0) {
                                    ...
                                    if (itemSpriteX > DrawingArea.topX - 32 && itemSpriteX < DrawingArea.bottomX
                                        ...
                                        if (sprite_2 != null) {
                                            ...
                                            }  else if (openInterfaceID == 5292 && child.invStackSizes[spriteIndex] == 0 || openInterfaceID == 12110 && child.invStackSizes[spriteIndex] == 0) {
                                                sprite_2.drawTransparentSprite(itemSpriteX, itemSpriteY, 90);
                                            } else {
                                                if (openInterfaceID == 58000 && child.invStackSizes[spriteIndex] == 0) {
                                            ...
                                            if (child.inv != null && openInterfaceID == 23100) {
                                                if (child.invItems[spriteIndex].getCustomValue() > 0) {
                                                    smallText.method385(0xFFFFFF, Misc.formatToRs(child.invItems[spriteIndex].getCustomValue()),
                                                            itemSpriteY + 35 + j7, itemSpriteX + k6 + 6);
                                                }
                                            }
                                            ...
                                            if (sprite_2.maxWidth == 33 || child.invStackSizes[spriteIndex] > 1) {
                                                ...
                                                if (!bankTab) {
                                                    if (k10 >= 1500000000 && child.drawInfinity) {
                                                        spriteLoader.lookup(653).drawSprite(itemSpriteX, itemSpriteY);
                                                    } else {
                                                        ...
                                                        if (k10 > 99999 && k10 < 10000000) {
                                                            ...
                                                            smallText.drawOpacityText(0xFFFF00, Integer.toString(0), itemSpriteY + 9 + j7, j7 + k6, 90);
                                                        } else {
                        ...
                        boolean verticle = child.verticleBar;
                        DrawingArea.drawFilledPixels(childX, childY, child.percentageDimension, child.height,
                                child.colorEmpty);
                        DrawingArea.drawFilledPixels(childX, childY, complete, child.height, child.colorFull);
                        if(child.drawPercentageBarBorder) {
                            DrawingArea.fillPixels(childX, child.percentageDimension, child.height, 0x000000, childY);
                        }
                        if(child.drawPercentageText) {
                            if (verticle) {
                                int xWidth = child.width;
                                font.drawCenteredString(percentage + "%", childX + (xWidth / 2),
                                        childY + 20 + size + (child.percentageDimension) + 6, 0xffffff, 0);
                            } else {
                                if (child.id == 20294) {
                                    font.drawCenteredString(percentage + "%", childX + (child.percentageDimension / 2) + 35,
                                            childY + (child.height / 2) + 2 + size, ColorConstants.ORANGE, 0);
                                } else if (openInterfaceID == 37500) {
                                    font.drawCenteredString(percentage + "%", childX + (child.percentageDimension / 2) - 7,
                                            childY + (child.height / 2) + 2 + size, 0xffffff, 0);
                                } else {
                                    if (openInterfaceID != 19490) {
                    ...
                    } else if (child.type == 4 || child.type == 17 || child.type == 23) {
                        TextDrawingArea textDrawingArea = child.textDrawingAreas;
                        String s;
                        if (child.type == 17) {
                            s = RSInterface.getWrappedText(textDrawingArea, child.message, child.width);
                        } else if(child.type == 23){
                            if(child.timeLeft == 0 || child.timeLeft - System.currentTimeMillis() < 0) {
                                continue;
                            }
                            s = DurationFormatUtils.formatDuration(child.timeLeft-System.currentTimeMillis(), child.message, true);
                        } else {
                            s = child.message;
                        }
                        int xOffset = 0;
                        ...
                        if (rsInterface.id == backDialogID || rsInterface.id == dialogID) {
                            if (color == 0xffff00) {
                                color = 0x0000ff;
                            }
                            if (color == 49152) {
                            ...
                            if (sprite != null) {
                                if (child.type == 5) {
                                    if (child.advancedSprite) {
                                        ...
                                        if(child.id == 19545) {
                                            sprite.drawSpriteWithOpacity(childX, childY, 45);
                                        } else if(child.highDetail) {
                                            sprite.drawAdvancedSprite(childX, childY);
                                        } else {
                                ...
                                if (child.id == 60003) {
                                    ...
                                    if (maxCapeColors != null) {
                                        ...
                                        if (maxCapeColor != null && maxCapeSlot != -1 && maxCapeSlot != -1) {
                                            ...
                                            RSInterface.interfaceCache.get(maxCapeInterfaceId).enabledColor = maxCapeColor.getRGB();
                                            maxCapeColor = null;
                                        }
                        ...
                        if (child.widgetActive) {
                            if (mouseX >= childX && mouseY >= childY && mouseX < childX + child.width
                                    && mouseY < childY + child.height) {
                                child.sprite2.drawSprite(childX, childY);
                            } else {
                                child.sprite1.drawSprite(childX, childY);
                            }
                            if(child.message.length() > 0) {
                                if (child.centerText)
                                    // childInterface.textDrawingAreas.drawText(colour, txt, x, y);
                                    child.textDrawingAreas.drawCenteredText(child.textColor,
                                            childX + child.messageOffsetX, child.message,
                                            childY + child.messageOffsetY, true);
                                else
                                    child.textDrawingAreas.drawText(child.textColor, child.message,
                                            childX + 5 + child.messageOffsetX, childY + child.messageOffsetY);
                            }
                        }
                        ...
                        Rasterizer.fillPixels(childX - 1, child.width + 2, child.height + 2, 0x383023, childY - 1);
                        Rasterizer.fillRect(childX, childY, child.width, child.height, child.color, 100);
                    } else if (child.type == 14) {
                        if (child.disabledSprite == null && !child.builtBackground) {
                            child.disabledSprite = RSInterface.buildBackground(child.width, child.height, child.backgroundDivider);
                            child.builtBackground = true;
                        }
                        ...
                        final Sprite sprite = spriteLoader.lookup(interfaceIsSelected(child) ?
                                child.selectedSpriteId : child.beingHovered ? child.enabledSpriteId : child.disabledSpriteId);
                        if (sprite != null) {
                            sprite.drawSprite(childX, childY);
                        }
                        child.beingHovered = false;
                    } else if (child.type == 16) {
                        child.scratchSprite.drawSprite(childX, childY);
                    }
                }
                if (openInterfaceID == 10000) {
                    if (!reportBox2Selected) {
    ...
    private void readPlayerUpdateMask(int i, int j, Stream stream, Player player) {
        ...
        if ((i & 0x80) != 0) {
            ...
            byte rights = (byte) stream.readUnsignedByte();
            int gloColor = 0;// 0;
            byte ironman2 = (byte) stream.readUnsignedByte();
            ...
            if (player.name != null && player.visible) {
                ...
                if (!flag && isOnTutorialIsland == 0) {
                    try {
                        ...
                        player.textCycle = 150;
                        pushMessage(message, 2, (gloColor != 0 ? "@glo" + gloColor + "@" : "") + player.name, player.playerTitle, rights, ironman2);
                    } catch (Exception exception) {
        ...
        if ((i & 0x20) != 0) {
            long k1 = inStream.readQWord();
            int k2 = stream.readUnsignedByte();
            int icon = stream.readUnsignedByte();
            long soakDamage = inStream.readQWord();
            player.updateHitData(k2, k1, loopCycle, icon, soakDamage);
            player.loopCycleStatus = loopCycle + 300;
            player.constitution = player.currentHealth = inStream.readQWord();
            player.maxConstitution = player.maxHealth = inStream.readQWord();
        }
        if ((i & 0x200) != 0) {
            long l1 = inStream.readQWord();
    ...
    private void drawParallelWidgets() {
        ...
        for (RSInterface widget : this.parallelWidgetList) {
            if (widget != null) {
                ...
                switch (widget.id) {
                    case 28710:
                        if (RSInterface.interfaceCache.get(28714).message.length() < 1) {
                            continue;
                        }
    ...
    private void draw3dScreen() {
        if (screenOpacity != 0 && screenOpacity != 255) {
            RSInterface.interfaceCache.get(35556).width = clientWidth;
            RSInterface.interfaceCache.get(35556).height = clientHeight;
            RSInterface.interfaceCache.get(35556).opacity = (byte) screenOpacity;
            drawInterface(0, 0, RSInterface.interfaceCache.get(35555), 0);
            drawInterface(0, 512, RSInterface.interfaceCache.get(35555), 0);
            drawInterface(0, 512 * 2, RSInterface.interfaceCache.get(35555), 0);
            drawInterface(0, 512 * 3, RSInterface.interfaceCache.get(35555), 0);
            drawInterface(0, 0, RSInterface.interfaceCache.get(35555), 700);
            drawInterface(0, 512, RSInterface.interfaceCache.get(35555), 700);
            drawInterface(0, 512 * 2, RSInterface.interfaceCache.get(35555), 700);
            drawInterface(0, 512 * 3, RSInterface.interfaceCache.get(35555), 700);
        }
        overlayTimersMgr.draw();
        drawParallelWidgets();
        //drawTeleIcon();
        ...
        this.broadCast.drawBroadCastMessage();
        if (getOption("rare_drops")) {
            processInterfaceAnimation(cycleTimer, 73650);
            drawInterface(0, 3, RSInterface.interfaceCache.get(73650), 18);
            if (RareDrop.showing) {
                RareDrop.display();
            }
        }
        if (clientSize != 0 && (walkableInterfaceId == 21119 || walkableInterfaceId == 21100)) {
            ...
            drawInterface(0, 0, RSInterface.interfaceCache.get(walkableInterfaceId), 0);
            ...
            drawInterface(0, interfaceX, RSInterface.interfaceCache.get(walkableInterfaceId), interfaceY);
        }

        if (openInterfaceID != -1) {
            processInterfaceAnimation(cycleTimer, openInterfaceID);
            if (clientSize == 0) {
                drawInterface(0, 0, RSInterface.interfaceCache.get(openInterfaceID), 0);
            } else {
                drawInterface(0, getOnScreenWidgetOffsets().x, RSInterface.interfaceCache.get(openInterfaceID), getOnScreenWidgetOffsets().y);
            }
            if (openInterfaceID == 5292) {
                if (bankItemDragSprite != null) {
        ...
        if (itemHover > -1 && (clientSize == 0 ? super.mouseX < gameAreaWidth : super.mouseX < gameAreaWidth - 200)) {
            if (controlIsDown && ItemStats.itemstats[itemHover] != null && ItemStats.itemstats[itemHover].type == 1) {
                drawInterface(0, getItemHoverX(false, 0, false), RSInterface.interfaceCache.get(94000),
                        getItemHoverY(false, 0, true));
            } else if (ItemStats.itemstats[itemHover] != null && ItemStats.itemstats[itemHover].type == 3) {
                drawInterface(0, getItemHoverX(false, 0, false), RSInterface.interfaceCache.get(94085),
                        getItemHoverY(false, 0, false));
            } else {
                drawInterface(0, getItemHoverX(false, 0, true), RSInterface.interfaceCache.get(94020),
                        getItemHoverY(false, 0, true));
            }
        }
        ...
        if (coinToggle) {
            int x = getCoinOrbX();
            int y = getCoinOrbY();
            spriteLoader.lookup(647).drawSprite(clientSize == 0 ? 447 : x - 67, y + 2);
            if (clientSize > 0) {
                spriteLoader.lookup(592).drawSprite(x, y);
            }
            if (RSInterface.interfaceCache.get(8135).message != null) {
                String cash = getMoneyInPouch();
                x = clientSize == 0 ? 485 : x - 29;
                newRegularFont.drawCenteredString("" + cash, x, y + 21,
                        getAmountColor((long) Long.parseLong(RSInterface.interfaceCache.get(8135).message)), 0);
            }
        }
        drawGrandExchange();
        if (!menuOpen) {
        ...
        if (drawMultiwayIcon == 1) {
            ...
            spriteLoader.lookup(652).drawSprite(clientSize == 0 ? 472 : clientWidth - 40, y);
        }
        if (fpsOn) {
            char c = '\u01FB';
            int k = 20;
        ...
        if (clientData) {
            ...
            final int regionX = x >> 6;
            final int regionY = y >> 6;
            final int regionId = regionX * 256 + regionY;
            drawingArea.method385(0xffff00, "Region Id: " + regionId + ";", 272 - minus, 5);
    ...
    private void determineMenuSize() {
        ...
        if (clientSize == 0) {
            if (super.saveClickX > 4 && super.saveClickY > 4 && super.saveClickX < 516 && super.saveClickY < 338) {
                ...
                worldController.clickedOnMap(plane, super.saveClickY, super.saveClickX, false);
            }
    ...
    private void animateModelOnInterface(RSInterface rsi, int duration, int animationId) {
        if (animationId != -1) {
            Animation seq = Animation.anims[animationId];
            for (rsi.frameTimer += duration; rsi.frameTimer > seq.getFrameLength(rsi.currentFrame); ) {
                rsi.frameTimer -= seq.getFrameLength(rsi.currentFrame) + 1;
                rsi.currentFrame++;
                if (rsi.currentFrame >= seq.frameCount) {
                    rsi.currentFrame -= seq.loopDelay;
                    if (rsi.currentFrame < 0 || rsi.currentFrame > seq.frameIDs.length - 1) {
                        rsi.currentFrame = 0;
                    }
                }
            }
    ...
    private boolean processInterfaceAnimation(int i, int interfaceId) {
        boolean flag1 = false;
        RSInterface rsi = RSInterface.interfaceCache.get(interfaceId);
        if (rsi == null) {
            return false;
        }
        if (rsi.children == null) {
        ...
        for (int childId = 0; childId < rsi.children.length; childId++) {
            ...
            RSInterface child = RSInterface.interfaceCache.get(rsi.children[childId]);
            if (child.type == 1) {
    ...
    private int extractInterfaceValues(RSInterface rsi, int j) {
        ...
        try {
            ...
            do {
                ...
                if (checkType == 4) {
                    RSInterface interfaceToCheckOn = RSInterface.interfaceCache.get(myValues[valueIdx++]);
                 ...
                 */
                if (checkType == 10) {
                    RSInterface itemInterface = RSInterface.interfaceCache.get(myValues[valueIdx++]);
    ...
    private void calcParticleScreenPos(int x, int offset, int z, int size) {
        ...
        int y = getFloorDrawHeight(plane, z, x) - offset;
        x -= xCameraPos;
        y -= zCameraPos;
        z -= yCameraPos;
        ...
        if (z >= 50) {
            particleDrawX = Rasterizer.textureInt1 + x * Rasterizer.fieldOfView / z;
            particleDrawY = Rasterizer.textureInt2 + y * Rasterizer.fieldOfView / z;
            particleDrawX2 = Rasterizer.textureInt1 + (x + size) * Rasterizer.fieldOfView / z;
            particleDrawY2 = Rasterizer.textureInt2 + (y + size) * Rasterizer.fieldOfView / z;
            particleDrawZ = z;
        } else {
            particleDrawX = -1;
            particleDrawY = -1;
    ...
    private void connectToUpdateServer() {
        ...
        while (expectedCRCs[8] == 0) {
            ...
            setLoadingText(10, "Connecting to Decimate...");
            try {
    ...
    public void drawLoginScreen() {
        ...
        spriteLoader.lookup(594).drawAdvancedSprite(0, 0);
        spriteLoader.lookup(1008).drawAdvancedSprite(257, loginScreenCursorPos == 0 ? 224 : 285);
        ...
        if (loginButtonHover) {
            spriteLoader.lookup(596).drawAdvancedSprite(276, 384);
        } else if (discordButtonHover) {
            spriteLoader.lookup(1041).drawAdvancedSprite(404, 379);
        } else if (youtubeButtonHover) {
            spriteLoader.lookup(1042).drawAdvancedSprite(468, 381);
        }

        if (loginScreenCursorPos == 0 && loopCycle % 45 < 10) {
            chatTextDrawingArea.drawRegularText(true, 293, 16777215, myUsername + "|", 297);
        } else {
            chatTextDrawingArea.drawRegularText(true, 293, 16777215, myUsername, 297);
        }

        if (loginScreenCursorPos == 1 && loopCycle % 45 < 10) {
            chatTextDrawingArea.drawRegularText(true, 293, 16777215, getStars(myPassword) + "|", 347);
        } else {
            chatTextDrawingArea.drawRegularText(true, 293, 16777215, getStars(myPassword), 347);
        }

        if (Configuration.DEBUG && !loggedIn) {
            chatTextDrawingArea.drawRegularText(true, 20, 16777215, "Mouse X: " + super.mouseX + " , Mouse Y: " + super.mouseY, 20);
        }

        titleScreen.drawGraphics(super.graphics);
    }
        ...
        try {
            if (super.clickMode3 == 1) {
                if (input1Hover) {
                    ...
                    RSFrame.openURL(Configuration.DISCORD_INVITE_LINK);
                } else if (youtubeButtonHover) {
                    RSFrame.openURL(Configuration.YOUTUBE_LINK);
                }
                saveSettings();
                if (loggedIn) {
                    return;
    ...
    public void sendFrame126(String str, int i) {
        if (i < RSInterface.interfaceCache.size() && RSInterface.interfaceCache.get(i) == null) {
            return;
        }
        try {
            RSInterface.interfaceCache.get(i).message = str;
            if (RSInterface.interfaceCache.get(i).parentID == tabInterfaceIDs[tabID]) {
                needDrawTabArea = true;
            }
        } catch (Exception e) {
    ...
    public void sendMysteryBoxComplete() {
        stream.createFrame(83);
    }

    public void sendPacket185(int buttonID) {
        if(this.variousSettings[2525] != 1 && buttonID == 3546) {
            return;
        }
        if(buttonID == 3546 && this.variousSettings[2525] == 1) {
            this.variousSettings[2525] = 0;
        }
        if(this.variousSettings[2525] != 1 && buttonID == 3546) {
            return;
        ...
        RSInterface rsi = RSInterface.interfaceCache.get(buttonID);
        if (rsi == null) {
            return;
        }
    ...
    public void sendPacket185(int button, int toggle, int type) {
        switch (type) {
            case 135:
                RSInterface class9 = RSInterface.interfaceCache.get(button);
                boolean flag8 = true;
                if (class9.contentType > 0) {
                    flag8 = promptUserForInput(class9);
                }
            ...
            case 646:
                stream.createFrame(185);
                stream.writeWord(button);
                RSInterface class9_2 = RSInterface.interfaceCache.get(button);
                if (class9_2.valueIndexArray != null && class9_2.valueIndexArray[0][0] == 5) {
            ...
            case 169:
                ...
                stream.createFrame(185);
                stream.writeWord(button);
                RSInterface clickedInterface = RSInterface.interfaceCache.get(button);
        ...
        try {
            ...
            switch (opCode) {
                ...
                case 161:
                    ...
                    RSInterface rsi = RSInterface.interfaceCache.get(frameId);
                ...
                case 176:
                    ...
                    if (anInt1193 != 0 && openInterfaceID == -1) {
                        ...
                        for (int k9 = 0; k9 < RSInterface.interfaceCache.size(); k9++) {
                            if (RSInterface.interfaceCache.get(k9) == null || RSInterface.interfaceCache.get(k9).contentType != c) {
                                continue;
                            }
                            openInterfaceID = RSInterface.interfaceCache.get(k9).parentID;
                ...
                case 185:
                    ...
                    RSInterface.interfaceCache.get(k).mediaType = 3;
                    if (myPlayer.desc == null) {
                        RSInterface.interfaceCache.get(k).mediaID = (myPlayer.anIntArray1700[0] << 25)
                                + (myPlayer.anIntArray1700[4] << 20) + (myPlayer.equipment[0] << 15)
                                + (myPlayer.equipment[8] << 10) + (myPlayer.equipment[11] << 5) + myPlayer.equipment[1];
                    } else {
                        RSInterface.interfaceCache.get(k).mediaID = (int) (0x12345678L + myPlayer.desc.type);
                    }
                    opCode = -1;
                    return true;
                ...
                case 72:
                    int i1 = inStream.ig2();
                    RSInterface class9 = RSInterface.interfaceCache.get(i1);
                    if (class9.inv == null) {
                        opCode = -1;
                        return true;
                    }
...
//                    }
                ...
                case 70:
                    ...
                    RSInterface rsi_select = RSInterface.interfaceCache.get(interface_id);
                ...
                case 209:
                    int interfaceId_ = inStream.readInt();
                    int newId = inStream.readUnsignedWord();

                    RSInterface.interfaceCache.get(interfaceId_).displayedSprite = spriteLoader.lookup(newId);
                    opCode = -1;
                    return true;
                ...
                case 208:
                    ...
                    RSInterface widget = RSInterface.interfaceCache.get(interfaceId);
                    if (widget != null) {
                        if (add) {
                ...
                case 205:
                    ...
                    RSInterface iface = RSInterface.interfaceCache.get(41022);
                    RSInterface iface2 = RSInterface.interfaceCache.get(7801);
                ...
                case 75:
                    ...
                    System.out.println("INT: " + j11);
                    RSInterface.interfaceCache.get(j11).mediaType = 2;
                    RSInterface.interfaceCache.get(j11).mediaID = mediaId;
                    opCode = -1;
                    return true;
                ...
                case 78:
                    ...
                    return true;
                case 252:
                    String spamMessage = inStream.readString();

                        pushMessage(spamMessage, 23, "");
                    opCode = -1;
                    return true;
                case 253:
                    ...
                    if (s.endsWith(":tradereq:")) {
                        ...
                        if (clan) {
                            ...
                            int length = Integer.parseInt(s.substring(8, 10));
                            s = s.substring(10, s.length());
                            String originals = s;
                            s = bracketColor + s.substring(0, 1) + "@blu@" + s.substring(1, length)
                                    + bracketColor + s.substring(length, length + 1)
                                    + CLAN_CHAT_COLORS[clanChatColor] + "<shad=0>" + s.substring(length + 1);
                        }
                ...
                case 112:
                    ironman = (byte) inStream.readUByte();
                    opCode = -1;
                    return true;
                ...
                case 82: // mystery box
                    int reward = inStream.readUnsignedWord();
                    int rewardAmount = inStream.readUnsignedByte();
                    if (rewardAmount == 255) {
                        rewardAmount = inStream.method440();
                    }

                    int totalItemAmount = inStream.readUnsignedByte();
                    MysteryBoxInterface.rewards = new int[totalItemAmount][2];
                    for (int index = 0; index < totalItemAmount; index++) {
                        int itemId = inStream.getUnsignedShort();
                        int itemAmount = inStream.readUnsignedByte();
                        if (itemAmount == 255) {
                            itemAmount = inStream.method440();
                        }
                        MysteryBoxInterface.rewards[index][0] = itemId;
                        MysteryBoxInterface.rewards[index][1] = itemAmount;
                    }
                    MysteryBoxInterface.openMysteryBox(reward, rewardAmount);
                    opCode = -1;
                    return true;

                case 80:
                    int inteface_id = inStream.readUnsignedWord();
                    int finalScroll = inStream.readUnsignedWord();
                    RSInterface interfaceToEdit = RSInterface.interfaceCache.get(inteface_id);
                    if (interfaceToEdit != null && interfaceToEdit.type == 0) {
                        if (finalScroll < 0) {
                            finalScroll = 0;
                        }
                        interfaceToEdit.scrollMax = finalScroll;
                    }
                    opCode = -1;
                    return true;

                case 79:
                    int j5 = inStream.readUnsignedWord();
                    int l12 = inStream.readUnsignedWord();
                    RSInterface class9_3 = RSInterface.interfaceCache.get(j5);
                ...
                case 204:
                    int kills = inStream.readInt();
                    int dAmount = inStream.readInt();
                    int dId = inStream.readInt();
                    String playerName = inStream.readString();
                    RareDrop drop = new RareDrop(dId, playerName, kills, dAmount);
                    RareDrop.addDrop(drop);
                    opCode = -1;
                    return true;
                ...
                case 196:
                    ...
                    inStream.readDWord();
                    byte playerRights = (byte) inStream.readUnsignedByte();
                    byte gameMode = (byte) inStream.readUnsignedByte();
                    boolean flag5 = false;
                    if (playerRights <= 1) {
                    ...
                    if (!flag5) {
                        try {
                            String message = TextInput.decodeToString(pktSize - 14, inStream);
                            if (playerRights != 0) {
                                pushMessage(message, 7, TextClass.fixName(TextClass.nameForLong(l5)), "", playerRights, gameMode);
                            } else {
                                pushMessage(message, 3, TextClass.fixName(TextClass.nameForLong(l5)), "", playerRights, gameMode);
                            }
                ...
                case 28:
                    ...
                    return true;
                case 29:
                    int dest = inStream.readUnsignedByte();
                    shadowProcessing = true;
                    if (dest == 250 || dest == 0) {
                        dest = 255;
                    }
                    shadowDestination = dest;
                    opCode = -1;
                    return true;
                case 246:
                    int i6 = inStream.ig2();
                    ...
                    if (k18 == 65535) {
                        RSInterface.interfaceCache.get(i6).mediaType = 0;
                        opCode = -1;
                        return true;
                    } else {
                        ItemDef itemDef = ItemDef.forID(k18);
                        RSInterface.interfaceCache.get(i6).mediaType = 4;
                        RSInterface.interfaceCache.get(i6).mediaID = k18;
                        RSInterface.interfaceCache.get(i6).modelRotation1 = itemDef.rotationY;
                        RSInterface.interfaceCache.get(i6).modelRotation2 = itemDef.rotationX;
                        RSInterface.interfaceCache.get(i6).modelZoom = (itemDef.modelZoom * 100) / i13;
                        opCode = -1;
                        return true;
                    }
                ...
                case 171:
                    ...
                    int j13 = inStream.readInt();
                    RSInterface.interfaceCache.get(j13).interfaceShown = flag1;
                    RSInterface.interfaceCache.get(j13).hiddenChild = flag1;
                    opCode = -1;
                    return true;

                case 231:
                    int npc_id = inStream.readShort();
                    int interfaceId1 = inStream.readInt();
                    final RSInterface npcOnInterface = RSInterface.interfaceCache.get(interfaceId1);
                ...
                case 236:
                    int parentId = inStream.getUnsignedShort();
                    int childIndex = inStream.method438();
                    int newChildId = inStream.getUnsignedShort();
                    RSInterface parent = RSInterface.interfaceCache.get(parentId);
                    if (parent != null && parent.children != null
                            && childIndex >= 0 && childIndex < parent.children.length) {
                        parent.children[childIndex] = newChildId;
                    }
                    opCode = -1;
                    return true;
                ...
                case 143:
                    this.broadCast.addMessage(inStream.readString());
                    opCode = -1;
                    return true;
                ...
                case 127:
                    myRights = (byte) inStream.readUnsignedByte();
                    opCode = -1;
                    return true;
                case 10:
                ...
                case 126:
                    ...
                    if (frame == 5385 && text.equals("scrollreset")) {
                        RSInterface.interfaceCache.get(5385).scrollPosition = 0;
                        opCode = -1;
                        return true;
                    }
                    if (frame == 29450) {
                    ...
                    if (text.startsWith("[REG]")) {
                        ...
                        RSInterface.interfaceCache.get(frame
                                + 20000).disabledSprite = RSInterface.interfaceCache.get(frame + 20000).savedSprite[0];
                    ...
                    if (text.startsWith("[OWN]")) {
                        text = text.substring(4);
                        RSInterface.interfaceCache.get(frame
                                + 20000).disabledSprite = RSInterface.interfaceCache.get(frame + 20000).savedSprite[1];
                        updateStrings(text, frame);
                        sendFrame126(text, frame);
                        opCode = -1;
                        return true;
                    ...
                    if (text.startsWith("[MOD]")) {
                        text = text.substring(4);
                        RSInterface.interfaceCache.get(frame
                                + 20000).disabledSprite = RSInterface.interfaceCache.get(frame + 20000).savedSprite[2];
                        updateStrings(text, frame);
                        sendFrame126(text, frame);
                        opCode = -1;
                        return true;
                    ...
                    if (text.startsWith("[REC]")) {
                        text = text.substring(4);
                        RSInterface.interfaceCache.get(frame
                                + 20000).disabledSprite = RSInterface.interfaceCache.get(frame + 20000).savedSprite[3];
                        updateStrings(text, frame);
                        sendFrame126(text, frame);
                        opCode = -1;
                        return true;
                    ...
                    if (text.startsWith("[COR]")) {
                        text = text.substring(4);
                        RSInterface.interfaceCache.get(frame
                                + 20000).disabledSprite = RSInterface.interfaceCache.get(frame + 20000).savedSprite[4];
                        updateStrings(text, frame);
                        sendFrame126(text, frame);
                        opCode = -1;
                        return true;
                    ...
                    if (text.startsWith("[SER]")) {
                        text = text.substring(4);
                        RSInterface.interfaceCache.get(frame
                                + 20000).disabledSprite = RSInterface.interfaceCache.get(frame + 20000).savedSprite[5];
                        updateStrings(text, frame);
                        sendFrame126(text, frame);
                        opCode = -1;
                        return true;
                    ...
                    if (text.startsWith("[LIE]")) {
                        text = text.substring(4);
                        RSInterface.interfaceCache.get(frame
                                + 20000).disabledSprite = RSInterface.interfaceCache.get(frame + 20000).savedSprite[6];
                        updateStrings(text, frame);
                        sendFrame126(text, frame);
                        opCode = -1;
                        return true;
                    ...
                    if (text.startsWith("[BER]")) {
                        text = text.substring(4);
                        RSInterface.interfaceCache.get(frame
                                + 20000).disabledSprite = RSInterface.interfaceCache.get(frame + 20000).savedSprite[7];
                        updateStrings(text, frame);
                        sendFrame126(text, frame);
                        opCode = -1;
                        return true;
                    ...
                    if (text.startsWith("[VR]")) {
                        ...
                        RSInterface.interfaceCache.get(frame
                                + 20000).disabledSprite = RSInterface.interfaceCache.get(frame + 20000).savedSprite[8];
                        updateStrings(text, frame);
                        sendFrame126(text, frame);
                        opCode = -1;
                        return true;
                    ...
                    if (text.startsWith("[FRI]")) {
                        text = text.substring(4);
                        RSInterface.interfaceCache.get(frame
                                + 20000).disabledSprite = RSInterface.interfaceCache.get(frame + 20000).savedSprite[9];
                        updateStrings(text, frame);
                        sendFrame126(text, frame);
                        opCode = -1;
                        return true;
                ...
                case 8:
                    ...
                    RSInterface.interfaceCache.get(k6).mediaType = 1;
                    RSInterface.interfaceCache.get(k6).mediaID = l13;
                    opCode = -1;
                    return true;
                ...
                case 122:
                    ...
                    int i22 = i14 >> 5 & 0x1f;
                    int l24 = i14 & 0x1f;
                    RSInterface.interfaceCache.get(rsi_id).disabledColor = (i19 << 19) + (i22 << 11) + (l24 << 3);
                    opCode = -1;
                    return true;
                case 129:
                    int progressBarPercentage = inStream.readUnsignedByte();
                    int childProgressBar = inStream.readInt();
                    RSInterface.interfaceCache.get(childProgressBar).percentageCompleted = progressBarPercentage;
                    opCode = -1;
                    return true;
                case 128:
                    int progressBarPercentage1 = inStream.readInt();
                    int progressBarTotalPercentage1 = inStream.readInt();
                    int childProgressBar1 = inStream.readInt();
                    RSInterface.interfaceCache.get(childProgressBar1).percentageCompleted = progressBarPercentage1;
                    RSInterface.interfaceCache.get(childProgressBar1).percentageTotal = progressBarTotalPercentage1;
                    opCode = -1;
                    return true;
                case 53:
                    needDrawTabArea = true;
                    try {
                        ...
                        RSInterface rsi_1 = RSInterface.interfaceCache.get(rsi_frame);
                        int totalItems = inStream.readUnsignedWord();
                        if (rsi_1 == null || rsi_1.inv == null || rsi_1.invStackSizes == null) {
                            opCode = -1;
                            return true;
                        }
                        int it = -1;
                        for (int idx = 0; idx < totalItems; idx++) {
                            ItemDto dto = new ItemDto(inStream);
                            rsi_1.inv[idx] = dto.getId();
                            rsi_1.invStackSizes[idx] = dto.getAmount();
                            rsi_1.invItems[idx] = dto;
                        }
                        for (int idx = totalItems; idx < rsi_1.inv.length && idx < rsi_1.invStackSizes.length; idx++) {
                            rsi_1.inv[idx] = 0;
                            rsi_1.invStackSizes[idx] = 0;
                            rsi_1.invItems[idx] = null;
                        }
                        if (rsi_frame == 24680) {
                            currentGEItem = it;
                        }
                    ...
                    return true;
                case 192:
                    String youtubeType = inStream.readString();
                ...
                case 228:
                    // needDrawTabArea = true;
                    try {
                        RSInterface rsi_1 = RSInterface.interfaceCache.get(70000);
                        int itemTot = inStream.readUnsignedWord();
                        int it = -1;
                        TextDrawingArea fonts[] = {smallText, drawingArea, chatTextDrawingArea};
                        int ch = 0;
                        int ch2 = 70001;
                        int y = 0;
                        rsi_1.scrollMax = ((50 * itemTot) - 2) < 400 ? 400 : ((50 * itemTot) - 2);
                        rsi_1.totalChildren((8 * itemTot));
                        for (int idx = 0; idx < itemTot; idx++) {
                            ...
                            long price = inStream.readQWord();

                            String owner = inStream.readString();

                            ItemDef d = ItemDef.forID(it - 1);
                            RSInterface.addSpriteLoader(ch2, 1147);

                            RSInterface.addText(ch2 + 1, "x" + itemAmt + " " + d.name, 0xffffff, false,
                                    true, 52, fonts, 0);
                            RSInterface.addText(ch2 + 2, ESFormat(price), 0xffffff, false, true, 52, fonts, 0);

                            RSInterface.addText(ch2 + 3, "Seller: " + owner, 0xffffff, false, true, 52, fonts, 0);

                            RSInterface.addHoverButtonWSpriteLoader(ch2 + 4, 1145, 42, 16, "Buy", -1, ch2 + 5, 1);
                            RSInterface.addHoveredImageWSpriteLoader(ch2 + 5, 1146, 42, 16, ch2 + 6);

                            RSInterface.addText(ch2 + 7, "Buy", 0xffffff, true, true, 52, fonts, 0);

                            rsi_1.child(ch, ch2, 0, y);

                            rsi_1.child(ch + 1, ch2 + 1, 45, y + 2);

                            rsi_1.child(ch + 2, ch2 + 2, 45, y + 17);

                            rsi_1.child(ch + 3, ch2 + 3, 45, y + 32);

                            rsi_1.child(ch + 4, ch2 + 4, 332, y + 17);

                            rsi_1.child(ch + 5, ch2 + 5, 332, y + 17);

                            rsi_1.child(ch + 6, ch2 + 7, 353, y + 19);

                            RSInterface.addItemOnInterface(ch2 + 8, 995,
                                    new String[]{null, null, null, null, null});

                            rsi_1.child(ch + 7, ch2 + 8, 8, y + 8);

                            RSInterface.interfaceCache.get(ch2 + 8).inv[0] = it;
                            RSInterface.interfaceCache.get(ch2 + 8).invStackSizes[0] = itemAmt;

                            for (int idp = 1; idp < RSInterface.interfaceCache.get(ch2 + 8).inv.length
                                    && idp < RSInterface.interfaceCache.get(ch2 + 8).invStackSizes.length; idp++) {
                                RSInterface.interfaceCache.get(ch2 + 8).inv[idp] = 0;
                                RSInterface.interfaceCache.get(ch2 + 8).invStackSizes[idp] = 0;
                            }
                            ch += 8;
                            ch2 += 9;
                            y += 50;
                ...
                case 54:
                    ...
                    try {
                        int rsi_frame = inStream.readUnsignedWord();
                        RSInterface rsi_1 = RSInterface.interfaceCache.get(rsi_frame);
                        if (rsi_1.inv == null && rsi_1.inv.length == 0 || rsi_1.invStackSizes == null && rsi_1.invStackSizes.length == 0) {
                            opCode = -1;
                            return true;
                        }
                        int totalItems = rsi_1.inv.length;
                        int it = -1;
                        for (int idx = 0; idx < totalItems; idx++) {
                            rsi_1.inv[idx] = 0;
                            rsi_1.invStackSizes[idx] = 0;
                ...
                case 230:
                    ...
                    RSInterface.interfaceCache.get(j14).modelRotation1 = k19;
                    RSInterface.interfaceCache.get(j14).modelRotation2 = k22;
                    RSInterface.interfaceCache.get(j14).modelZoom = j7;
                    opCode = -1;
                    return true;
                ...
                case 200:
                    ...
                    RSInterface class9_4 = RSInterface.interfaceCache.get(l8);
                ...
                case 34:
                    ...
                    RSInterface class9_2 = RSInterface.interfaceCache.get(i9);
                    while (inStream.currentOffset < pktSize) {
                ...
                case 162:
                    int identifier = inStream.readShort();
                    int spriteId = inStream.readShort();
                    int secondsDuration = inStream.readShort();
                    int value = inStream.readShort();
                    boolean isItem = inStream.readUnsignedByte() == 1;
                    if (identifier == 0 && value == 0
                            || identifier == 1 && secondsDuration == 0) {
                        this.overlayTimersMgr.remove(identifier, spriteId);
                    } else {
                        this.overlayTimersMgr.addNewOverlay(identifier, spriteId, secondsDuration, value, isItem);
                    }
                    opCode = -1;
                    return true;
    ...
    private void renderWorld() {
        ...
        DrawingArea.resetImage(CACHED_FOG_SETTING);
    ...
    private boolean clickedAttackOption;
    private Entity currentEntityInteraction;
    private long lastEntityInteraction;

    private void drawEntityOverlay() {
        if (currentEntityInteraction == null) {
            return;
        }

        if (System.currentTimeMillis() - lastEntityInteraction > 1000) {
            currentEntityInteraction = null;

            return;
        }

        long currentHealth = currentEntityInteraction.currentHealth;
        long maxHealth = currentEntityInteraction.maxHealth;

        String name = null;
        if (currentEntityInteraction instanceof Player) {
            name = ((Player) currentEntityInteraction).name;
        } else if (currentEntityInteraction instanceof NPC) {
            if (((NPC) currentEntityInteraction).desc != null) {
                name = removeTags(((NPC) currentEntityInteraction).desc.name);
            }
        }
        ...
        final RSFontSystem font = newBoldFont;
        int width = font.getTextWidth(name) + 15;
        if (width < 170) {
            width = 170;
        }

        int height = 44;
        DrawingArea.fillRectangle(0x463d32, y, width, height, 180, x);
        DrawingArea.drawRectangle(y, height, 255, 0x5a5245, width, x);
        DrawingArea.drawRectangle(y - 1, height, 255, 0x383023, width + 2, x - 1);

        font.drawCenteredString(name, (width / 2) + 5, y + 15, 0xffffff, 0);
        int percent = (int) (((double) currentHealth / (double) maxHealth) * (width - 4));
        if (percent >= width - 4) {
            percent = width - 4;
        }
        DrawingArea.fillRectangle(0xff0000, y + 20, width - 4, 20, 160, x + 2);
        DrawingArea.fillRectangle(0x00ff00, y + 20, percent - 4, 20, 160, x + 2);
        DrawingArea.drawRectangle(y + 20, 20, 255, 0x383023, width - 4, x + 2);
        font.drawCenteredString(currentHealth + " / " + maxHealth, (width / 2) + 5, y + 35, 0xffffff, 0);
    }
    ...
    public final String formatAmount(long amount) {
        if (amount > 99999 && amount <= 9999999) {
            ...
            return amount / 1000000000000000L + "Q";
    ...
    public final String formatPouchAmount(long amount) {
        if (amount > 0 && amount <= 999) {
            return amount + "B";
        } else if (amount > 999 && amount <= 999999) {
            return amount / 1000 + "T";
        } else if (amount >= 1000000 && amount < 1000000000L) {
            return amount / 1000000 + "Q";
        } else if (amount >= 1000000000L && amount < 1000000000000L) {
            return amount / 1000000000 + "QT";
        } else if (amount >= 1000000000000L && amount <= 1000000000000000L) {
            return amount / 1000000000000L + "ST";
        } else if (amount >= 1000000000000000L && amount < 1000000000000000000L) {
            return amount / 1000000000000000L + "SPT";
        } else if (amount >= 1000000000000000000L && amount < Long.MAX_VALUE) {
            return amount / 1000000000000000000L + "OCT";
        }
        return Long.toString(amount);
    }
    ...
    public static String ESFormat(int bonus) {
        double bonus2 = bonus;
        String fin = "";
        if (bonus2 > 1000000000) {
            bonus2 /= 1000000000;
            fin = String.format("%.2f", bonus2) + "B";
        } else if (bonus2 > 1000000) {
            bonus2 /= 1000000;
            fin = String.format("%.2f", bonus2) + "M";
        } else if (bonus2 > 1000) {
            bonus2 /= 1000;
            fin = String.format("%.2f", bonus2) + "K";
        } else {
            fin += String.format("%.2f", bonus2);
        }
        return fin;
    }

    public static String ESFormat(long bonus) {
        double bonus2 = bonus;
        String fin = "";
        if (bonus2 > 1000000000000000l) {
            bonus2 /= 1000000000000000l;
            fin = String.format("%.2f", bonus2) + "Q";
        } else if (bonus2 > 1000000000000l) {
            bonus2 /= 1000000000000l;
            fin = String.format("%.2f", bonus2) + "T";
        } else if (bonus2 > 1000000000) {
            bonus2 /= 1000000000;
            fin = String.format("%.2f", bonus2) + "QT";
        } else if (bonus2 > 1000000) {
            bonus2 /= 1000000;
            fin = String.format("%.2f", bonus2) + "Q";
        } else if (bonus2 > 1000) {
    ...
    public void drawGrandExchange() {
        ...
        if (openInterfaceID == 54700) {
            ...
            if (slotColorPercent[slotSelected] == 100 || slotAborted[slotSelected]) {
                RSInterface.interfaceCache.get(54800).tooltip = "[GE]";
            } else {
                RSInterface.interfaceCache.get(54800).tooltip = "Abort offer";
            }
            if (slotSelected <= 6) {
        ...
        if (openInterfaceID == 53700) {
            ...
            abort2 = SpriteCache.spriteCache[651];
            if (slotColorPercent[slotSelected] == 100 || slotAborted[slotSelected]) {
                RSInterface.interfaceCache.get(53800).tooltip = "[GE]";
            } else {
                RSInterface.interfaceCache.get(53800).tooltip = "Abort offer";
            }
            if (slotSelected <= 6) {
                if (!slotAborted[slotSelected]) {
                    for (int k9 = 1; k9 < slotColorPercent[slotSelected]; k9++) {
    ...
    public void drawUpdate(int id, String type) {
        ...
        if (type == "Sell") {
            ...
            drawInterface(0, x + 110, RSInterface.interfaceCache.get(54000), y + 38);
            ...
            drawPercentage(id);
            smallText.method592(0xCC9900, x2, RSInterface.interfaceCache.get(32000 + id).message, y2 - minus, true);
            smallText.method592(0xBDBB5B, x2, RSInterface.interfaceCache.get(33000 + id).message, y2, true);
            smallText.method592(0xFFFF00, x3, RSInterface.interfaceCache.get(33100 + id).message, y3, true);
            setHovers(id, false);
    ...
    public void setGrandExchange(int id, boolean on) {
        switch (id) {
            case 1:
                if (on) {
                    RSInterface.interfaceCache.get(24505).tooltip = "Buy";
                    RSInterface.interfaceCache.get(24511).tooltip = "Sell";
                    changeSet(id, false, false);
                } else {
                    RSInterface.interfaceCache.get(24505).tooltip = "[GE]";
                    RSInterface.interfaceCache.get(24511).tooltip = "[GE]";
                }
                break;
            case 2:
                if (on) {
                    RSInterface.interfaceCache.get(24523).tooltip = "Buy";
                    RSInterface.interfaceCache.get(24526).tooltip = "Sell";
                    changeSet(id, false, false);
                } else {
                    RSInterface.interfaceCache.get(24523).tooltip = "[GE]";
                    RSInterface.interfaceCache.get(24526).tooltip = "[GE]";
                }
                break;
            case 3:
                if (on) {
                    RSInterface.interfaceCache.get(24514).tooltip = "Buy";
                    RSInterface.interfaceCache.get(24529).tooltip = "Sell";
                    changeSet(id, false, false);
                } else {
                    RSInterface.interfaceCache.get(24514).tooltip = "[GE]";
                    RSInterface.interfaceCache.get(24529).tooltip = "[GE]";
                }
                break;
            case 4:
                if (on) {
                    RSInterface.interfaceCache.get(24508).tooltip = "Buy";
                    RSInterface.interfaceCache.get(24532).tooltip = "Sell";
                    changeSet(id, false, false);
                } else {
                    RSInterface.interfaceCache.get(24508).tooltip = "[GE]";
                    RSInterface.interfaceCache.get(24532).tooltip = "[GE]";
                }
                break;
            case 5:
                if (on) {
                    RSInterface.interfaceCache.get(24517).tooltip = "Buy";
                    RSInterface.interfaceCache.get(24535).tooltip = "Sell";
                    changeSet(id, false, false);
                } else {
                    RSInterface.interfaceCache.get(24517).tooltip = "[GE]";
                    RSInterface.interfaceCache.get(24535).tooltip = "[GE]";
                }
                break;
            case 6:
                if (on) {
                    RSInterface.interfaceCache.get(24520).tooltip = "Buy";
                    RSInterface.interfaceCache.get(24538).tooltip = "Sell";
                    changeSet(id, false, false);
                } else {
                    RSInterface.interfaceCache.get(24520).tooltip = "[GE]";
                    RSInterface.interfaceCache.get(24538).tooltip = "[GE]";
                }
                break;
        }
    }
    ...
    public void changeSet(int id, boolean view, boolean abort) {
        switch (id) {
            case 1:
                if (view) {
                    RSInterface.interfaceCache.get(24543).tooltip = "View offer";
                } else {
                    RSInterface.interfaceCache.get(24543).tooltip = "[GE]";
                }
                if (abort) {
                    RSInterface.interfaceCache.get(24541).tooltip = "Abort offer";
                } else {
                    RSInterface.interfaceCache.get(24541).tooltip = "[GE]";
                }
                break;
            case 2:
                if (view) {
                    RSInterface.interfaceCache.get(24547).tooltip = "View offer";
                } else {
                    RSInterface.interfaceCache.get(24547).tooltip = "[GE]";
                }
                if (abort) {
                    RSInterface.interfaceCache.get(24545).tooltip = "Abort offer";
                } else {
                    RSInterface.interfaceCache.get(24545).tooltip = "[GE]";
                }
                break;
            case 3:
                if (view) {
                    RSInterface.interfaceCache.get(24551).tooltip = "View offer";
                } else {
                    RSInterface.interfaceCache.get(24551).tooltip = "[GE]";
                }
                if (abort) {
                    RSInterface.interfaceCache.get(24549).tooltip = "Abort offer";
                } else {
                    RSInterface.interfaceCache.get(24549).tooltip = "[GE]";
                }
                break;
            case 4:
                if (view) {
                    RSInterface.interfaceCache.get(24555).tooltip = "View offer";
                } else {
                    RSInterface.interfaceCache.get(24555).tooltip = "[GE]";
                }
                if (abort) {
                    RSInterface.interfaceCache.get(24553).tooltip = "Abort offer";
                } else {
                    RSInterface.interfaceCache.get(24553).tooltip = "[GE]";
                }
                break;
            case 5:
                if (view) {
                    RSInterface.interfaceCache.get(24559).tooltip = "View offer";
                } else {
                    RSInterface.interfaceCache.get(24559).tooltip = "[GE]";
                }
                if (abort) {
                    RSInterface.interfaceCache.get(24557).tooltip = "Abort offer";
                } else {
                    RSInterface.interfaceCache.get(24557).tooltip = "[GE]";
                }
                break;
            case 6:
                if (view) {
                    RSInterface.interfaceCache.get(24563).tooltip = "View offer";
                } else {
                    RSInterface.interfaceCache.get(24563).tooltip = "[GE]";
                }
                if (abort) {
                    RSInterface.interfaceCache.get(24561).tooltip = "Abort offer";
                } else {
                    RSInterface.interfaceCache.get(24561).tooltip = "[GE]";
                }
                break;
        }
    }
    ...
    private boolean dialogueDisplayed() {
        return backDialogID > 0 || dialogID > 0 || showInput || inputDialogState > 0 || aString844 != null;
    }

    private int tabToReplyIndex = 0;
    private final List<String> tabToReplyList = new ArrayList<>();

    private void tabToReplyPm() {
        if (dialogueDisplayed() && tabToReplyList.size() > 1) {
            tabToReplyIndex++;
        }
        ...
        for (int i = 0; i < 500; i++) {
            if (chatMessages[i] != null) {
                ...
                if (type == 3 || type == 7) {
                    final String chatName = removeCrownTags(chatNames[i]);
                    if (!tabToReplyList.contains(chatName)) {
                        // Add a new private-message member to start of
                        // list.
                        tabToReplyList.add(0, chatName);

                        // Limit the list to 5 members.
                        final int lastIndex = tabToReplyList.size() - 1;
                        if (tabToReplyList.size() > 5) {
                            tabToReplyList.remove(lastIndex);
                        }
                    } else {
                        // If the next member to private-message us is already
                        // in the list, set them to the front of the list, whilst removing
                        // from their old index.
                        final int oldIndex = tabToReplyList.indexOf(chatName);
                        tabToReplyList.add(0, tabToReplyList.remove(oldIndex));
                    }
                    break;
                }
            }
        ...
        if (tabToReplyList.isEmpty()) {
            tabToReplyIndex = 0;
            pushMessage("You haven't received any messages to which you can reply.", 0, "");
            return;
        }

        if (tabToReplyIndex >= tabToReplyList.size()) {
            tabToReplyIndex = 0;
        }

        final String name = tabToReplyList.get(tabToReplyIndex);
        privateMessageReply(name, TextClass.longForName(name.trim()));
    }

    private void privateMessageReply(String name, long nameAsLong) {
        int id = -1;
        for (int i = 0; i < friendsCount; i++) {
        ...
        final boolean isFriend = id != -1;
        if (isFriend && friendsNodeIDs[id] == 0) {
        ...
        friendsListAction = 3;
        aLong953 = nameAsLong;
        promptMessage = "Type a message to send to " + name + (!isFriend ? " (not on your friends list)" : "");
    }

    private String removeCrownTags(String str) {
        if (str.startsWith("@cr0@") || str.startsWith("@cr1@") || str.startsWith("@cr2@")
                || str.startsWith("@cr3@") || str.startsWith("@cr4@") || str.startsWith("@cr5@")
                || str.startsWith("@cr6@") || str.startsWith("@cr7@") || str.startsWith("@cr8@")
                || str.startsWith("@cr9@")) {
            return str.substring(5);
        } else if (str.startsWith("@cr10@") || str.startsWith("@cr11@") || str.startsWith("@cr12@")
                || str.startsWith("@cr13@")) {
    ...
    public void hitmarkDraw(Entity e, int hitLength, int type, int icon, long damage, long soak, int move, int opacity,
                            int mask) {
        int drawPos = 0;
        if (mask == 0) {
        ...
        if (damage > 0) {
            ...
            (type == 1 ? bigHit : smallHit).drawOpacityText(0xffffff, String.valueOf(damage),
                    drawPos + (type == 1 ? 2 : 32), spriteDrawX + (String.valueOf(damage).length() >= 5 ? 16 : 4) + (soak > 0 ? -16 : 0), opacity);
            ...
            spriteLoader.lookup(656).drawSprite3(spriteDrawX - decrX, drawPos - 14, opacity);
            if (soak > 0) {
    ...
    public void sendConsoleCommand(String command) {
        ...
        switch (commandStart) {
            case "debug":
                RSInterface.achievementsInterface();
            ...
            if (getQuickPrayersSet() > 0) {
                if (!quickPrsActive) {
                    for (int i = 0; i < quickPrayers.length; i++) {
                        ...
                        RSInterface rsInterface = RSInterface.interfaceCache.get(button);
    ...
    public void togglePrayerState(int button) {
        ...
        if (prayerInterfaceType == 5608) {
            if ((currentMaxStats[5]) >= prayerLevelRequirements[index]) {
            ...
            if ((currentMaxStats[5]) >= curseLevelRequirements[index]) {
    ...
    private void turnOffPrayers() {
        ...
        for (int i = 0; i < quickPrayers.length; i++) {
            ...
            ;
            RSInterface rsInterface = RSInterface.interfaceCache.get(x);
            if (rsInterface.valueIndexArray != null && rsInterface.valueIndexArray[0][0] == 5) {
                toggle = rsInterface.valueIndexArray[0][1];
    ...
    private void turnOffCurses() {
        int toggle = -1;
        for (int i = 0; i < quickCurses.length; i++) {
            RSInterface rsInterface = RSInterface.interfaceCache.get((i * 2) + 32503);
            if (rsInterface.valueIndexArray != null && rsInterface.valueIndexArray[0][0] == 5) {
                toggle = rsInterface.valueIndexArray[0][1];
    ...
    public void drawOnBankInterface() {
        if (openInterfaceID == 5292 && RSInterface.interfaceCache.get(27000).message.equals("1")) {
            int i = Integer.parseInt(RSInterface.interfaceCache.get(27001).message);
            int j = Integer.parseInt(RSInterface.interfaceCache.get(27002).message);
            for (int k = 0; k <= i; k++) {
                RSInterface.interfaceCache.get(27014 + k).disabledSprite = spriteLoader.lookup(927);
                RSInterface.interfaceCache.get(27014 + k).message = (new StringBuilder())
                        .append("Click here to select tab ").append(k + 1).toString();
            }

            for (int l = i + 1; l <= 8; l++) {
                RSInterface.interfaceCache.get(27014 + l).disabledSprite = new Sprite("");
                RSInterface.interfaceCache.get(27014 + l).message = "";
            }

            if (i != 8) {
                RSInterface.interfaceCache.get(27015 + i).disabledSprite = spriteLoader.lookup(926);
                RSInterface.interfaceCache.get(27015 + i).message = "Drag an item here to create a new tab";
            }
            if (j == -1) {
                RSInterface.interfaceCache.get(27013).disabledSprite = spriteLoader.lookup(929);
            } else if (j > 0) {
                RSInterface.interfaceCache.get(27014 + j).disabledSprite = spriteLoader.lookup(930);
                RSInterface.interfaceCache.get(27014).disabledSprite = spriteLoader.lookup(929);
            } else {
                RSInterface.interfaceCache.get(27014).disabledSprite = spriteLoader.lookup(925);
            }
            RSInterface.interfaceCache.get(27000).message = "0";
        }
    }
    ...
    public void drawCoinOrb() {
        ...
        if (hover) {
            spriteLoader.lookup(630).drawSprite(x, y);
        } else {
            spriteLoader.lookup(592).drawSprite(x, y);
        }
        if (clientSize == 0) {
            spriteLoader.lookup(593).drawSprite(-1, y);
        }
    }

    public String getMoneyInPouch() {
        String Cash = RSInterface.interfaceCache.get(8135).message;
        long convertedMoney = Long.parseLong(Cash);
        Cash = formatPouchAmount(convertedMoney);
        return Cash;
    }
    ...
    public void drawArrow(String title, String information, int x, int y, int speed, int pause, int type) {
        ...
        int low = 0;
        int rt = type;
        spriteLoader.lookup(648 + rt).drawAdvancedSprite2(x, y, arrowO);
    ...
    public void updateItemHoverInterface(ItemDef def) {
        String action = "";
        if (itemHover > 0 && def != null) {
            if (def.actions != null) {
                action = def.actions[1] == null ? "" : def.actions[1];
            }
            RSInterface.interfaceCache.get(94002).message = "@whi@" + action + " @lre@" + def.name;
            if (ItemStats.itemstats[itemHover] != null) {
                if (ItemStats.itemstats[itemHover].type == 1) {
                    RSInterface.interfaceCache.get(94006).message = "";
                    String bonus = "";
                    for (int i = 0; i < 5; ++i) {
                        if (ItemStats.itemstats[itemHover].attackBonus[i] > 0)
                            ...
                            bonus = "";
                        RSInterface.interfaceCache.get(94006).message += "@whi@" + bonus + ""
                                + ItemStats.itemstats[itemHover].attackBonus[i] + "\\n";
                    }
                    RSInterface.interfaceCache.get(94007).message = "";
                    for (int i = 0; i < 5; ++i) {
                        if (ItemStats.itemstats[itemHover].defenceBonus[i] > 0)
                            bonus = "+";
                        else
                            bonus = "";
                        RSInterface.interfaceCache.get(94007).message += "@whi@" + bonus + ""
                                + ItemStats.itemstats[itemHover].defenceBonus[i] + "\\n";
                    }
                    RSInterface.interfaceCache.get(94010).message = "@whi@"
                    ...
                    RSInterface.interfaceCache.get(94021).height = 28;
                    RSInterface.interfaceCache.get(94021).width = 145;
                    RSInterface.interfaceCache.get(94020).childX[1] = 80;
                    RSInterface.interfaceCache.get(94022).message = "@whi@Press " + "@lre@CTRL"
                            + "@whi@ to view stats";
                    RSInterface.interfaceCache.get(94022).centerText = false;
                    RSInterface.interfaceCache.get(94020).childX[2] = 10;
                    ...
                    if (width < 58)
                        width = 58;
                    RSInterface.interfaceCache.get(94021).width = width;
                    RSInterface.interfaceCache.get(94020).childX[1] = 8 + (width / 2);
                    RSInterface.interfaceCache.get(94020).childX[2] = 8 + (width / 2);
                    RSInterface.interfaceCache.get(94022).message = "@whi@Heals: @gre@"
                            + ItemStats.itemstats[itemHover].healAmount;
                    RSInterface.interfaceCache.get(94022).centerText = true;
                } else if (ItemStats.itemstats[itemHover].type == 3) {
                    /*RSInterface.setScrollableItems(RSInterface.interfaceCache.get(94089),
                            ItemStats.itemstats[itemHover].rewards);*/
                    RSInterface.interfaceCache.get(94087).message = ItemStats.itemstats[itemHover].information;
                }
            } else {
                RSInterface.interfaceCache.get(94021).height = 16;
                RSInterface.interfaceCache.get(94021).width = newSmallFont.getTextWidth(def.name)
                        + newSmallFont.getTextWidth(action) + 8;
                RSInterface.interfaceCache.get(94020).childX[1] = 8 + (RSInterface.interfaceCache.get(94021).width / 2);
                RSInterface.interfaceCache.get(94022).message = "";
            }
        }
    }
    public int getItemHoverX(boolean inventory, int dx, boolean check) {
        int x = super.mouseX - dx;
        int width = RSInterface.interfaceCache.get(94021).width;
        if (inventory) {
            if (x > 240 - width)
                x = 240 - width;
            if (clientSize != 0) {
    ...
    public int getItemHoverY(boolean inventory, int dy, boolean check) {
        ...
        int height = check ? RSInterface.interfaceCache.get(94021).height : 145;
        if (inventory) {
            ...
            if (clientSize != 0) {
                ...
                if (y + height > gameAreaHeight)
                    y = gameAreaHeight - height - 5;
            }
        } else {
            int max = check ? (gameAreaHeight) : (gameAreaHeight - 330);
            if (y > max)
                y = max;
        }
        return y;
    ...
    public Point getOnScreenWidgetOffsets() {
        int w = 512, h = 334;
        int x = (clientWidth / 2) - 256;
        int y = (clientHeight / 2) - 169;
        if (x + w > (clientWidth - 225)) {
            x = x - 90;
            if (x < 0) {
                x = 0;
