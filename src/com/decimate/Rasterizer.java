...
public final class Rasterizer extends DrawingArea {
    ...
    public static int MAX_TEXTURES = 132;
    ...
    private static int applyFogFilter(int rgb, float ratio) {
        if (!aBoolean1464 || !Client.CACHED_FOG_SETTING) {
            return rgb;
        }

        final int renderDistance = (WorldController.drawDistance * 140) - 500;
    ...
    public static void method368(CacheArchive archive) {
        File folder = new File(signlink.findcachedir(), "Textures");
        ...
        for (int i = 0; i < MAX_TEXTURES; i++) {
            try {
                ...
                if (file.exists()) {
                    aBackgroundArray1474s[i] = new Background(FileOperations.readFile(file.getAbsolutePath()));
                } else {
    ...
    private static int[] method371(int texture) {
        ...
        int[] rawPixels = null;
        if (aBackgroundArray1474s[texture] != null) {
            rawPixels = aBackgroundArray1474s[texture].getRawPixels();
        }
        ...
        if (background.imgWidth == 64) {
            for (int j1 = 0; j1 < 128; j1++) {
                for (int j2 = 0; j2 < 128; j2++) {
                    texels[j2 + (j1 << 7)] = (rawPixels == null ? ai1[background.imgPixels[(j2 >> 1) + (j1 >> 1 << 6)]]
                            : rawPixels[(j2 >> 1) + (j1 >> 1 << 6)]);
                }
            }
        } else {
            for (int k1 = 0; k1 < 16384; k1++) {
                texels[k1] = (rawPixels == null ? ai1[background.imgPixels[k1]] : rawPixels[k1]);
            }
        }
    ...
    public static void calculatePalette(double value, boolean dummy) {
