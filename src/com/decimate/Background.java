...
public final class Background extends DrawingArea {
    ...
    public Background(byte[] imageData) {
        final Image image = Toolkit.getDefaultToolkit().createImage(imageData);
        final ImageIcon sprite = new ImageIcon(image);
        this.libWidth = this.imgWidth = sprite.getIconWidth();
        this.libHeight = this.imgHeight = sprite.getIconHeight();
        this.rawPixels = new int[this.libWidth * this.libHeight];
        this.palette = new int[this.libWidth * this.libHeight];

        final PixelGrabber pixelGrabber = new PixelGrabber(image, 0, 0,
                this.libWidth, this.libHeight, this.rawPixels, 0, this.libWidth);
        try {
